Module [java.scripting](../../module-summary.md)

# Package javax.script

---

package javax.script

The scripting API consists of interfaces and classes that define
Java Scripting Engines and provides
a framework for their use in Java applications. This API is intended
for use by application programmers who wish to execute programs
written in scripting languages in their Java applications. The
scripting language programs are usually provided by the end-users of
the applications.

The main areas of functionality of `javax.script`
package include

1. **Script execution**: Scripts
   are streams of characters used as sources for programs executed by
   script engines. Script execution uses
   [`eval`](ScriptEngine.md#eval(java.lang.String,javax.script.ScriptContext)) methods of
   [`ScriptEngine`](ScriptEngine.md "interface in javax.script") and methods of the
   [`Invocable`](Invocable.md "interface in javax.script") interface.

   - **Binding**: This facility
     allows Java objects to be exposed to script programs as named
     variables. [`Bindings`](Bindings.md "interface in javax.script") and
     [`ScriptContext`](ScriptContext.md "interface in javax.script")
     classes are used for this purpose.

     - **Compilation**: This
       functionality allows the intermediate code generated by the
       front-end of a script engine to be stored and executed repeatedly.
       This benefits applications that execute the same script multiple
       times. These applications can gain efficiency since the engines'
       front-ends only need to execute once per script rather than once per
       script execution. Note that this functionality is optional and
       script engines may choose not to implement it. Callers need to check
       for availability of the [`Compilable`](Compilable.md "interface in javax.script")
       interface using an *instanceof* check.

       - **Invocation**: This
         functionality allows the reuse of intermediate code generated by a
         script engine's front-end. Whereas Compilation allows entire scripts
         represented by intermediate code to be re-executed, Invocation
         functionality allows individual procedures/methods in the scripts to
         be re-executed. As in the case with compilation, not all script
         engines are required to provide this facility. Caller has to check
         for [`Invocable`](Invocable.md "interface in javax.script") availability.

         - **Script engine discovery**: Applications
           written to the Scripting API might have specific requirements on
           script engines. Some may require a specific scripting language
           and/or version while others may require a specific implementation
           engine and/or version. Script engines are packaged in a specified
           way so that engines can be discovered at runtime and queried for
           attributes. The Engine discovery mechanism is based on the service-provider
           loading facility described in the [`ServiceLoader`](../../../java.base/java/util/ServiceLoader.md "class in java.util") class.
           [`ScriptEngineManager`](ScriptEngineManager.md "class in javax.script")
           includes
           [`getEngineFactories`](ScriptEngineManager.md#getEngineFactories()) method to get all
           [`ScriptEngineFactory`](ScriptEngineFactory.md "interface in javax.script") instances
           discovered using this mechanism. `ScriptEngineFactory` has
           methods to query attributes about script engine.

Since:
:   1.6

* All Classes and InterfacesInterfacesClassesException Classes

  Class

  Description

  [AbstractScriptEngine](AbstractScriptEngine.md "class in javax.script")

  Provides a standard implementation for several of the variants of the `eval`
  method.

  [Bindings](Bindings.md "interface in javax.script")

  A mapping of key/value pairs, all of whose keys are
  `Strings`.

  [Compilable](Compilable.md "interface in javax.script")

  The optional interface implemented by ScriptEngines whose methods compile scripts
  to a form that can be executed repeatedly without recompilation.

  [CompiledScript](CompiledScript.md "class in javax.script")

  Extended by classes that store results of compilations.

  [Invocable](Invocable.md "interface in javax.script")

  The optional interface implemented by ScriptEngines whose methods allow the invocation of
  procedures in scripts that have previously been executed.

  [ScriptContext](ScriptContext.md "interface in javax.script")

  The interface whose implementing classes are used to connect Script Engines
  with objects, such as scoped Bindings, in hosting applications.

  [ScriptEngine](ScriptEngine.md "interface in javax.script")

  `ScriptEngine` is the fundamental interface whose methods must be
  fully functional in every implementation of this specification.

  [ScriptEngineFactory](ScriptEngineFactory.md "interface in javax.script")

  `ScriptEngineFactory` is used to describe and instantiate
  `ScriptEngines`.

  [ScriptEngineManager](ScriptEngineManager.md "class in javax.script")

  The `ScriptEngineManager` implements a discovery and instantiation
  mechanism for `ScriptEngine` classes and also maintains a
  collection of key/value pairs storing state shared by all engines created
  by the Manager.

  [ScriptException](ScriptException.md "class in javax.script")

  The generic `Exception` class for the Scripting APIs.

  [SimpleBindings](SimpleBindings.md "class in javax.script")

  A simple implementation of Bindings backed by
  a `HashMap` or some other specified `Map`.

  [SimpleScriptContext](SimpleScriptContext.md "class in javax.script")

  Simple implementation of ScriptContext.