...
This class can be used with an arbitrary [`ColorSpace`](../color/ColorSpace.md "class in java.awt.color"). The number of
...
The transfer types supported are DataBuffer.TYPE\_BYTE,
...
* Masks are identified by indices running from 0 through
  [`getNumComponents`](ColorModel.md#getNumComponents()) - 1.* The first
    [`getNumColorComponents`](ColorModel.md#getNumColorComponents())
    indices refer to color samples.* If an alpha sample is present, it corresponds the last index.* The order of the color indices is specified
        by the `ColorSpace`. Typically, this reflects the name of
        the color space type (for example, TYPE\_RGB), index 0
...
[`SinglePixelPackedSampleModel`](SinglePixelPackedSampleModel.md "class in java.awt.image") to construct a
[`BufferedImage`](BufferedImage.md "class in java.awt.image"). Normally the masks used by the
[`SampleModel`](SampleModel.md "class in java.awt.image") and the `ColorModel` would be the same.
...
A subclass of `PackedColorModel` is [`DirectColorModel`](DirectColorModel.md "class in java.awt.image"),
...
* ## Method Summary
  ...
  Returns a [`WritableRaster`](WritableRaster.md "class in java.awt.image") representing the alpha channel of
...
* ## Constructor Details
  ...
  + ### PackedColorModel
    ...
    public PackedColorModel([ColorSpace](../color/ColorSpace.md "class in java.awt.color") space,
    int bits,
    int[] colorMaskArray,
    int alphaMask,
    boolean isAlphaPremultiplied,
  ...
  + ### PackedColorModel
    ...
    must be of type ColorSpace.TYPE\_RGB. All of the bits in each
