...
is described by a [`PathIterator`](geom/PathIterator.md "interface in java.awt.geom") object, which can express the
...
* it lies completely
  inside the `Shape` boundary *or** it lies exactly on the `Shape` boundary *and* the
    space immediately adjacent to the
    point in the increasing `X` direction is
    entirely inside the boundary *or** it lies exactly on a horizontal boundary segment **and** the
...
:   * [`PathIterator`](geom/PathIterator.md "interface in java.awt.geom")
    ...
    * [`FlatteningPathIterator`](geom/FlatteningPathIterator.md "class in java.awt.geom")
    * [`GeneralPath`](geom/GeneralPath.md "class in java.awt.geom")
...
* ## Method Summary
  ...
  [definition of insideness](#def_insideness).
  ...
  Tests if a specified [`Point2D`](geom/Point2D.md "class in java.awt.geom") is inside the boundary
...
* ## Method Details
  ...
  + ### getBounds
    ...
    [Rectangle](Rectangle.md "class in java.awt") getBounds()
    ...
    Note that the
    [definition of insideness](#def_insideness) can lead to situations where points
    ...
    [`contains(point)`](#contains(double,double)) method, then
    it must be inside the returned `Rectangle` bounds object
    according to the [`contains(point)`](#contains(double,double))
    ...
    See Also:
    :   - [`getBounds2D()`](#getBounds2D())
  + ### getBounds2D

    [Rectangle2D](geom/Rectangle2D.md "class in java.awt.geom") getBounds2D()
    ...
    If a `point` is inside the `shape` according to the
    [`contains(point)`](#contains(java.awt.geom.Point2D)) method, then it must
    be inside the returned `Rectangle2D` bounds object according
    to the [`contains(point)`](#contains(java.awt.geom.Point2D)) method of the
    `bounds`. Specifically:
  ...
  + ### contains
    ...
    boolean contains([Point2D](geom/Point2D.md "class in java.awt.geom") p)
  ...
  + ### intersects
    ...
    intersect the `Shape`.
    The [`Area`](geom/Area.md "class in java.awt.geom") class performs
    ...
    See Also:
    :   - [`Area`](geom/Area.md "class in java.awt.geom")
  + ### intersects

    boolean intersects([Rectangle2D](geom/Rectangle2D.md "class in java.awt.geom") r)
    ...
    See Also:
    :   - [`intersects(double, double, double, double)`](#intersects(double,double,double,double))
  + ### contains
  ...
  + ### contains
    ...
    boolean contains([Rectangle2D](geom/Rectangle2D.md "class in java.awt.geom") r)
    ...
    See Also:
    :   - [`contains(double, double, double, double)`](#contains(double,double,double,double))
  + ### getPathIterator

    [PathIterator](geom/PathIterator.md "interface in java.awt.geom") getPathIterator([AffineTransform](geom/AffineTransform.md "class in java.awt.geom") at)

    Returns an iterator object that iterates along the
    `Shape` boundary and provides access to the geometry of the
    `Shape` outline. If an optional [`AffineTransform`](geom/AffineTransform.md "class in java.awt.geom")
