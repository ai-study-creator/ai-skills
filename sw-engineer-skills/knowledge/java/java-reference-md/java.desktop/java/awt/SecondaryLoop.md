...
[`EventQueue.createSecondaryLoop()`](EventQueue.md#createSecondaryLoop()) method. The interface
provides two methods, [`enter()`](#enter()) and [`exit()`](#exit()),
which can be used to start and stop the event loop.

When the [`enter()`](#enter()) method is called, the current
thread is blocked until the loop is terminated by the
[`exit()`](#exit()) method. Also, a new event loop is started
on the event dispatch thread, which may or may not be
the current thread. The loop can be terminated on any
thread by calling its [`exit()`](#exit()) method. After the
...
See Also:
:   * [`Dialog.show()`](Dialog.md#show())
    * [`EventQueue.createSecondaryLoop()`](EventQueue.md#createSecondaryLoop())
...
* ## Method Summary
  ...
  Unblocks the execution of the thread blocked by the [`enter()`](#enter()) method and exits the secondary loop.
...
* ## Method Details
  ...
  + ### enter
    ...
    dispatch thread. This thread will be blocked until the [`exit()`](#exit()) method is called or the loop is terminated. A new
  ...
  + ### exit
    ...
    This method resumes the thread that called the [`enter()`](#enter())
    method and exits the secondary loop that was created when
    the [`enter()`](#enter()) method was invoked.
    ...
    If this secondary loop has not been started with the [`enter()`](#enter()) method, or this secondary loop has already finished
    with the [`exit()`](#exit()) method, this method returns `false`, otherwise `true` is returned.

    Returns:
    :   `true` if this loop was previously started and
        has not yet been finished with the [`exit()`](#exit()) method,
        `false` otherwise