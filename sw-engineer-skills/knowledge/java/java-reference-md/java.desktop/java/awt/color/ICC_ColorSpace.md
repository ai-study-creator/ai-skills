...
# Class ICC\_ColorSpace

[java.lang.Object](../../../../java.base/java/lang/Object.md "class in java.lang")

[java.awt.color.ColorSpace](ColorSpace.md "class in java.awt.color")

java.awt.color.ICC\_ColorSpace
...
public class ICC\_ColorSpace
extends [ColorSpace](ColorSpace.md "class in java.awt.color")

The `ICC_ColorSpace` class is an implementation of the abstract
`ColorSpace` class. This representation of device independent and
...
Typically, a `Color` or `ColorModel` would be associated with an
...
image, or device (see `ICC_Profile`). Attempting to create an
`ICC_ColorSpace` object from an inappropriate ICC Profile is an error.
...
directions. Should an `ICC_ColorSpace` object be used in a way
...
`ICC_ColorSpace` object will produce output in the specified type of
color space (e.g. `TYPE_RGB`, `TYPE_CMYK`, etc.), but the
...
default color spaces via [`ColorSpace.getInstance(int)`](ColorSpace.md#getInstance(int)).

See Also:
:   * [`ColorSpace`](ColorSpace.md "class in java.awt.color")
    * [`ICC_Profile`](ICC_Profile.md "class in java.awt.color")
    * [Serialized Form](../../../../serialized-form.md#java.awt.color.ICC_ColorSpace)

* ## Field Summary

  ### Fields inherited from class java.awt.color.[ColorSpace](ColorSpace.md "class in java.awt.color")
...
* ## Constructor Summary
  ...
  Constructs a new `ICC_ColorSpace` from an `ICC_Profile`
  object.
* ## Method Summary
  ...
  ### Methods inherited from class java.awt.color.[ColorSpace](ColorSpace.md "class in java.awt.color")
...
* ## Constructor Details
  ...
  + ### ICC\_ColorSpace
    ...
    public ICC\_ColorSpace([ICC\_Profile](ICC_Profile.md "class in java.awt.color") profile)
...
* ## Method Details
  ...
  + ### getProfile
    ...
    public [ICC\_Profile](ICC_Profile.md "class in java.awt.color") getProfile()
  ...
  + ### toCIEXYZ
    ...
    `CS_CIEXYZ` color space. This representation is useful in a
    two-step color conversion process in which colors are transformed from an
    input color space to `CS_CIEXYZ` and then to an output color space.
    ...
    space used in the methods `toCIEXYZ` and `fromCIEXYZ` is the
  ...
  + ### fromCIEXYZ
    ...
    the `CS_CIEXYZ` color space. This representation is useful in a
    two-step color conversion process in which colors are transformed from an
    input color space to `CS_CIEXYZ` and then to an output color space.
  ...
  + ### getMinValue
    ...
    Returns the minimum normalized color component value for the specified
    component. For `TYPE_XYZ` spaces, this method returns minimum
    values of 0.0 for all components. For `TYPE_Lab` spaces, this
    ...
    for all components. When using an `ICC_ColorSpace` with a profile
  ...
  + ### getMaxValue
    ...
    Returns the maximum normalized color component value for the specified
    component. For `TYPE_XYZ` spaces, this method returns maximum
    values of 1.0 + (32767.0 / 32768.0) for all components. For
    `TYPE_Lab` spaces, this method returns 100.0 for L and 127.0 for a
    ...
    `ICC_ColorSpace` with a profile that requires different maximum
