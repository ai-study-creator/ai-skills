...
public final class ICC\_ProfileGray
...
See Also:
:   * [Serialized Form](../../../../serialized-form.md#java.awt.color.ICC_ProfileGray)
...
* ## Method Details
  ...
  + ### getGamma
    ...
    obtained via `getTRC()`. When using a gamma value, the PCS Y
    ...
    Throws:
    :   `ProfileDataException` - if the profile does not specify the TRC as a
        single gamma value
  + ### getTRC
    ...
    throws an exception, and the `getGamma()` method should be used to
    ...
    index `length-1`. Interpolation may be used to generate output
