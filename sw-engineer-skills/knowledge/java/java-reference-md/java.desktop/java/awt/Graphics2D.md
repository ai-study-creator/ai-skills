...
extends [Graphics](Graphics.md "class in java.awt")
...
an [`AffineTransform`](geom/AffineTransform.md "class in java.awt.geom") object as part of its rendering state
...
[`GraphicsConfiguration`](GraphicsConfiguration.md "class in java.awt") object defines the characteristics
...
[`Component`](Component.md "class in java.awt") or [`Image`](Image.md "class in java.awt")). This default transform maps the
...
1. Determine what to render.- Constrain the rendering operation to the current `Clip`.
     The `Clip` is specified by a [`Shape`](Shape.md "interface in java.awt") in user
     ...
     *device clip*, which is defined by the visibility of windows and
     ...
     system to reflect the resulting composite clip.- Determine what colors to render.- Apply the colors to the destination drawing surface using the current
         [`Composite`](Composite.md "interface in java.awt") attribute in the `Graphics2D` context.
...
1. **`Shape` operations**
   1. If the operation is a `draw(Shape)` operation, then
      the [`createStrokedShape`](Stroke.md#createStrokedShape(java.awt.Shape))
      method on the current [`Stroke`](Stroke.md "interface in java.awt") attribute in the
      `Graphics2D` context is used to construct a new
      `Shape` object that contains the outline of the specified
      `Shape`.- The `Shape` is transformed from user space to device space
        using the current `Transform`
        in the `Graphics2D` context.- The outline of the `Shape` is extracted using the
          [`getPathIterator`](Shape.md#getPathIterator(java.awt.geom.AffineTransform)) method of
          `Shape`, which returns a
          [`PathIterator`](geom/PathIterator.md "interface in java.awt.geom")
          object that iterates along the boundary of the `Shape`.- If the `Graphics2D` object cannot handle the curved segments
            that the `PathIterator` object returns then it can call the
            alternate
            [`getPathIterator`](Shape.md#getPathIterator(java.awt.geom.AffineTransform,double))
            method of `Shape`, which flattens the `Shape`.- The current [`Paint`](Paint.md "interface in java.awt") in the `Graphics2D` context
              is queried for a [`PaintContext`](PaintContext.md "interface in java.awt"), which specifies the
              colors to render in device space.- **Text operations**
     1. The following steps are used to determine the set of glyphs required
        to render the indicated `String`:
        1. If the argument is a `String`, then the current
           ...
           algorithms the font implements.- If the argument is an
             ...
             [`TextLayout`](font/TextLayout.md "class in java.awt.font")
             ...
             for multiple fonts of differing writing directions.- If the argument is a
               [`GlyphVector`](font/GlyphVector.md "class in java.awt.font"), then the
               ...
               each glyph.- The current `Font` is queried to obtain outlines for the
          indicated glyphs. These outlines are treated as shapes in user space
          relative to the position of each glyph that was determined in step 1.- The character outlines are filled as indicated above
            under [`Shape` operations](#rendershape).- The current `Paint` is queried for a
              `PaintContext`, which specifies
              the colors to render in device space.- **`Image` Operations**
       1. The region of interest is defined by the bounding box of the source
          `Image`.
          This bounding box is specified in Image Space, which is the
          `Image` object's local coordinate system.- If an `AffineTransform` is passed to
            [`drawImage(Image, AffineTransform, ImageObserver)`](#drawImage(java.awt.Image,java.awt.geom.AffineTransform,java.awt.image.ImageObserver)),
            the `AffineTransform` is used to transform the bounding
            box from image space to user space. If no `AffineTransform`
            is supplied, the bounding box is treated as if it is already in user space.- The bounding box of the source `Image` is transformed from user
              space into device space using the current `Transform`.
              Note that the result of transforming the bounding box does not
              necessarily result in a rectangular region in device space.- The `Image` object determines what colors to render,
...
*`Paint`*: The color of the `Component`. *`Font`*: The `Font` of the `Component`. *`Stroke`*: A square pen with a linewidth of 1, no dashing, miter segment joins and square end caps. *`Transform`*: The [`getDefaultTransform`](GraphicsConfiguration.md#getDefaultTransform()) for the `GraphicsConfiguration` of the `Component`. *`Composite`*: The [`AlphaComposite.SRC_OVER`](AlphaComposite.md#SRC_OVER) rule. *`Clip`*: No rendering `Clip`, the output is clipped to the `Component`.
...
[`KEY_ANTIALIASING`](RenderingHints.md#KEY_ANTIALIASING) hint key
to the
[`VALUE_ANTIALIAS_OFF`](RenderingHints.md#VALUE_ANTIALIAS_OFF)
...
setting the
[`KEY_ANTIALIASING`](RenderingHints.md#KEY_ANTIALIASING) hint
key to
[`VALUE_ANTIALIAS_ON`](RenderingHints.md#VALUE_ANTIALIAS_ON)
...
* Device coordinates are defined to be between device pixels which
  ...
  pixels, whether or not it is rendered using antialiasing.* Lines and paths stroked using the `BasicStroke`
    ...
    [`KEY_STROKE_CONTROL`](RenderingHints.md#KEY_STROKE_CONTROL) hint.
...
* For `fill` operations, including `fillRect`,
  ...
  `clearRect`, [`fill`](#fill(java.awt.Shape)) can now be called
  ...
  is called.* Similarly, for draw operations, including `drawLine`,
    `drawRect`, `drawRoundRect`,
    `drawOval`, `drawArc`, `drawPolyline`,
    and `drawPolygon`, [`draw`](#draw(java.awt.Shape)) can now be
    ...
    is called.* The `draw3DRect` and `fill3DRect` methods were
...
1. The `setPaintMode` method is implemented as a convenience
   method to set the default `Composite`, equivalent to
   `setComposite(new AlphaComposite.SrcOver)`.- The `setXORMode(Color xorcolor)` method is implemented
...
* ## Method Summary
  ...
  [`GlyphVector`](font/GlyphVector.md "class in java.awt.font") using
  ...
  filtered with a
  [`BufferedImageOp`](image/BufferedImageOp.md "interface in java.awt.image").
  ...
  Renders a
  [`RenderableImage`](image/renderable/RenderableImage.md "interface in java.awt.image.renderable"),
  ...
  Renders a [`RenderedImage`](image/RenderedImage.md "interface in java.awt.image"),
  ...
  in accordance with the specification of the [`TextAttribute`](font/TextAttribute.md "class in java.awt.font") class.
  ...
  the specified [`Rectangle`](Rectangle.md "class in java.awt"), which is in device
  space.
  ...
  ### Methods inherited from class java.awt.[Graphics](Graphics.md "class in java.awt")
...
* ## Constructor Details
  ...
  + ### Graphics2D
    ...
    [`BufferedImage`](image/BufferedImage.md "class in java.awt.image") objects.

    See Also:
    :   - [`Component.getGraphics()`](Component.md#getGraphics())
        - [`Graphics.create()`](Graphics.md#create())
* ## Method Details
  ...
  + ### draw3DRect
    ...
    See Also:
    :   - [`Graphics.fill3DRect(int, int, int, int, boolean)`](Graphics.md#fill3DRect(int,int,int,int,boolean))
  + ### fill3DRect
    ...
    See Also:
    :   - [`Graphics.draw3DRect(int, int, int, int, boolean)`](Graphics.md#draw3DRect(int,int,int,int,boolean))
  + ### draw

    public abstract void draw([Shape](Shape.md "interface in java.awt") s)
    ...
    See Also:
    :   - [`setStroke(java.awt.Stroke)`](#setStroke(java.awt.Stroke))
        - [`setPaint(java.awt.Paint)`](#setPaint(java.awt.Paint))
        - [`Graphics.setColor(java.awt.Color)`](Graphics.md#setColor(java.awt.Color))
        - [`transform(java.awt.geom.AffineTransform)`](#transform(java.awt.geom.AffineTransform))
        - [`setTransform(java.awt.geom.AffineTransform)`](#setTransform(java.awt.geom.AffineTransform))
        - [`clip(java.awt.Shape)`](#clip(java.awt.Shape))
        - [`Graphics.setClip(int, int, int, int)`](Graphics.md#setClip(int,int,int,int))
        - [`setComposite(java.awt.Composite)`](#setComposite(java.awt.Composite))
  + ### drawImage
    ...
    [ImageObserver](image/ImageObserver.md "interface in java.awt.image") obs)
    ...
    :   `obs` - the [`ImageObserver`](image/ImageObserver.md "interface in java.awt.image")
  ...
  + ### drawImage
    ...
    public abstract void drawImage([BufferedImage](image/BufferedImage.md "class in java.awt.image") img,
    [BufferedImageOp](image/BufferedImageOp.md "interface in java.awt.image") op,
    int x,
    int y)
  ...
  + ### drawRenderedImage
    ...
    public abstract void drawRenderedImage([RenderedImage](image/RenderedImage.md "interface in java.awt.image") img,
    [AffineTransform](geom/AffineTransform.md "class in java.awt.geom") xform)
  ...
  + ### drawRenderableImage
    ...
    public abstract void drawRenderableImage([RenderableImage](image/renderable/RenderableImage.md "interface in java.awt.image.renderable") img,
    [AffineTransform](geom/AffineTransform.md "class in java.awt.geom") xform)
    ...
    and rendered using
    [`drawRenderedImage`](#drawRenderedImage(java.awt.image.RenderedImage,java.awt.geom.AffineTransform)).
    ...
    :   - [`transform(java.awt.geom.AffineTransform)`](#transform(java.awt.geom.AffineTransform))
        ...
        - [`clip(java.awt.Shape)`](#clip(java.awt.Shape))
        - [`Graphics.setClip(int, int, int, int)`](Graphics.md#setClip(int,int,int,int))
        - [`drawRenderedImage(java.awt.image.RenderedImage, java.awt.geom.AffineTransform)`](#drawRenderedImage(java.awt.image.RenderedImage,java.awt.geom.AffineTransform))
  + ### drawString
    ...
    See Also:
    :   - [`Graphics.drawBytes(byte[], int, int, int, int)`](Graphics.md#drawBytes(byte%5B%5D,int,int,int,int))
        - [`Graphics.drawChars(char[], int, int, int, int)`](Graphics.md#drawChars(char%5B%5D,int,int,int,int))
  + ### drawString
    ...
    :   - [`setPaint(java.awt.Paint)`](#setPaint(java.awt.Paint))
        - [`Graphics.setColor(java.awt.Color)`](Graphics.md#setColor(java.awt.Color))
        - [`Graphics.setFont(java.awt.Font)`](Graphics.md#setFont(java.awt.Font))
        - [`setTransform(java.awt.geom.AffineTransform)`](#setTransform(java.awt.geom.AffineTransform))
        - [`setComposite(java.awt.Composite)`](#setComposite(java.awt.Composite))
        - [`Graphics.setClip(int, int, int, int)`](Graphics.md#setClip(int,int,int,int))
  + ### drawString
  ...
  + ### drawGlyphVector
    ...
    public abstract void drawGlyphVector([GlyphVector](font/GlyphVector.md "class in java.awt.font") g,
    float x,
    float y)
    ...
    specifies individual glyphs from a [`Font`](Font.md "class in java.awt").
    ...
    See Also:
    :   - [`Font.createGlyphVector(java.awt.font.FontRenderContext, java.lang.String)`](Font.md#createGlyphVector(java.awt.font.FontRenderContext,java.lang.String))
        - [`GlyphVector`](font/GlyphVector.md "class in java.awt.font")
        - [`setPaint(java.awt.Paint)`](#setPaint(java.awt.Paint))
        - [`Graphics.setColor(java.awt.Color)`](Graphics.md#setColor(java.awt.Color))
        - [`setTransform(java.awt.geom.AffineTransform)`](#setTransform(java.awt.geom.AffineTransform))
        - [`setComposite(java.awt.Composite)`](#setComposite(java.awt.Composite))
  ...
  + ### fill
    ...
    public abstract void fill([Shape](Shape.md "interface in java.awt") s)
  ...
  + ### hit
    ...
    public abstract boolean hit([Rectangle](Rectangle.md "class in java.awt") rect,
    [Shape](Shape.md "interface in java.awt") s,
    boolean onStroke)
    ...
    See Also:
    :   - [`setStroke(java.awt.Stroke)`](#setStroke(java.awt.Stroke))
        - [`fill(java.awt.Shape)`](#fill(java.awt.Shape))
        - [`draw(java.awt.Shape)`](#draw(java.awt.Shape))
        - [`transform(java.awt.geom.AffineTransform)`](#transform(java.awt.geom.AffineTransform))
        - [`setTransform(java.awt.geom.AffineTransform)`](#setTransform(java.awt.geom.AffineTransform))
        - [`clip(java.awt.Shape)`](#clip(java.awt.Shape))
        - [`Graphics.setClip(int, int, int, int)`](Graphics.md#setClip(int,int,int,int))
  + ### getDeviceConfiguration
    ...
    public abstract [GraphicsConfiguration](GraphicsConfiguration.md "class in java.awt") getDeviceConfiguration()
  ...
  + ### setComposite
    ...
    public abstract void setComposite([Composite](Composite.md "interface in java.awt") comp)
    ...
    See Also:
    :   - [`Graphics.setXORMode(java.awt.Color)`](Graphics.md#setXORMode(java.awt.Color))
        - [`Graphics.setPaintMode()`](Graphics.md#setPaintMode())
  ...
  + ### setPaint
    ...
    public abstract void setPaint([Paint](Paint.md "interface in java.awt") paint)
  ...
  + ### setStroke
    ...
    public abstract void setStroke([Stroke](Stroke.md "interface in java.awt") s)
  ...
  + ### setRenderingHint
    ...
    public abstract void setRenderingHint([RenderingHints.Key](RenderingHints.Key.md "class in java.awt") hintKey,
    [Object](../../../java.base/java/lang/Object.md "class in java.lang") hintValue)
    ...
    See Also:
    :   - [`getRenderingHint(RenderingHints.Key)`](#getRenderingHint(java.awt.RenderingHints.Key))
        - [`RenderingHints`](RenderingHints.md "class in java.awt")
  + ### getRenderingHint

    public abstract [Object](../../../java.base/java/lang/Object.md "class in java.lang") getRenderingHint([RenderingHints.Key](RenderingHints.Key.md "class in java.awt") hintKey)
    ...
    See Also:
    :   - [`RenderingHints`](RenderingHints.md "class in java.awt")
        - [`setRenderingHint(RenderingHints.Key, Object)`](#setRenderingHint(java.awt.RenderingHints.Key,java.lang.Object))
  + ### setRenderingHints

    public abstract void setRenderingHints([Map](../../../java.base/java/util/Map.md "interface in java.util")<?,?> hints)

    Replaces the values of all preferences for the rendering
    algorithms with the specified `hints`.
    The existing values for all rendering hints are discarded and
    the new set of known hints and values are initialized from the
    specified [`Map`](../../../java.base/java/util/Map.md "interface in java.util") object.
  ...
  + ### addRenderingHints
    ...
    public abstract void addRenderingHints([Map](../../../java.base/java/util/Map.md "interface in java.util")<?,?> hints)
  ...
  + ### getRenderingHints
    ...
    public abstract [RenderingHints](RenderingHints.md "class in java.awt") getRenderingHints()
    ...
    See Also:
    :   - [`RenderingHints`](RenderingHints.md "class in java.awt")
        - [`setRenderingHints(Map)`](#setRenderingHints(java.util.Map))
  + ### translate
  ...
  + ### transform
    ...
    public abstract void transform([AffineTransform](geom/AffineTransform.md "class in java.awt.geom") Tx)
  ...
  + ### setTransform
    ...
    public abstract void setTransform([AffineTransform](geom/AffineTransform.md "class in java.awt.geom") Tx)
  ...
  + ### getTransform
    ...
    public abstract [AffineTransform](geom/AffineTransform.md "class in java.awt.geom") getTransform()
  ...
  + ### getPaint
    ...
    public abstract [Paint](Paint.md "interface in java.awt") getPaint()
  ...
  + ### getComposite
    ...
    public abstract [Composite](Composite.md "interface in java.awt") getComposite()
  ...
  + ### setBackground
    ...
    public abstract void setBackground([Color](Color.md "class in java.awt") color)
    ...
    :   - [`getBackground()`](#getBackground())
        - [`Graphics.clearRect(int, int, int, int)`](Graphics.md#clearRect(int,int,int,int))
  + ### getBackground

    public abstract [Color](Color.md "class in java.awt") getBackground()
  ...
  + ### getStroke
    ...
    public abstract [Stroke](Stroke.md "interface in java.awt") getStroke()
  ...
  + ### clip
    ...
    public abstract void clip([Shape](Shape.md "interface in java.awt") s)
    ...
    [`setClip`](Graphics.md#setClip(java.awt.Shape)) with a `null`
    argument, the specified `Shape` becomes the new
    user clip.

    Since this method intersects the specified shape
    with the current clip, it will throw `NullPointerException`
    for a `null` shape unless the user clip is also `null`.
    So calling this method with a `null` argument is not recommended.

    Parameters:
    :   `s` - the `Shape` to be intersected with the current
        `Clip`. This method updates the current `Clip`.
  ...
  + ### getFontRenderContext
    ...
    public abstract [FontRenderContext](font/FontRenderContext.md "class in java.awt.font") getFontRenderContext()

    Get the rendering context of the `Font` within this
    `Graphics2D` context.
    The [`FontRenderContext`](font/FontRenderContext.md "class in java.awt.font")
