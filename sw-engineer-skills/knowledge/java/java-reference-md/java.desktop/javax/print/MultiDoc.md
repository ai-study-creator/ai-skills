...
implements interface `MultiDoc`, and the Print Job calls methods on
that object to obtain the print data.

Interface `MultiDoc` provides an abstraction similar to a "linked list"
...
list. The Print Job can call the multidoc's [`getDoc()`](#getDoc()) method
to get the current doc. When it's ready to go on to the next doc, the Print
Job can call the multidoc's [`next()`](#next()) method to get the next
...
Of course, interface `MultiDoc` can be implemented in any way that
...
1. The **interleaved** pattern: Get the doc from the current multidoc.
   ...
   example above uses the interleaved pattern.)- The **all-at-once** pattern: Get the doc from the current multidoc,
...
interleaved pattern. That is, given a `MultiDoc` object, the print
service proxy will call [`getDoc()`](#getDoc()) one or more times until it
successfully obtains the current `Doc` object. The print service proxy
...
continue, the print service proxy will then call [`next()`](#next()) one
...
interface `MultiDoc` can assume the print service proxy will follow
this interleaved pattern; for any other pattern of usage, the
`MultiDoc` implementation's behavior is unspecified.
...
* ## Method Details
  ...
  + ### getDoc
    ...
    [Doc](Doc.md "interface in javax.print") getDoc()
    ...
    Throws:
    :   `IOException` - if an error occurred when reading the document
  + ### next

    [MultiDoc](MultiDoc.md "interface in javax.print") next()
    throws [IOException](../../../java.base/java/io/IOException.md "class in java.io")
    ...
    Returns:
    :   multidoc object containing the next doc object, or `null`
        if there are no further doc objects

    Throws:
    :   `IOException` - if an error occurred locating the next document