...
`SecurityManager.checkPrintJobAccess()` to deny access to untrusted
...
Services which are registered by [`registerService(PrintService)`](#registerService(javax.print.PrintService)) will
...
* ## Method Summary
  ...
  combination of attributes and `DocFlavors` specified.
  ...
  `static final MultiDocPrintService[]`

  `lookupMultiDocPrintServices(DocFlavor[] flavors,
  AttributeSet attributes)`

  Locates `MultiDoc` print `Services` capable of printing
  `MultiDocs` containing all the specified doc flavors.
...
* ## Method Details
  ...
  + ### lookupPrintServices
    ...
    public static final [PrintService](PrintService.md "interface in javax.print")[] lookupPrintServices([DocFlavor](DocFlavor.md "class in javax.print") flavor,
    ...
    Parameters:
    :   `flavor` - the flavor to print. If `null`, this constraint is
        not used.
    :   `attributes` - attributes that the print service must support. If
        `null` this constraint is not used.

    Returns:
    :   array of matching `PrintService` objects representing print
  ...
  + ### lookupMultiDocPrintServices
    ...
    public static final [MultiDocPrintService](MultiDocPrintService.md "interface in javax.print")[] lookupMultiDocPrintServices([DocFlavor](DocFlavor.md "class in javax.print")[] flavors,
    [AttributeSet](attribute/AttributeSet.md "interface in javax.print.attribute") attributes)
    ...
    `MultiDoc` in which the elements may be different flavors. An
    application could perform this itself by multiple lookups on each
    `DocFlavor` in turn and collating the results, but the lookup
    ...
    Returns:
    :   array of matching [`MultiDocPrintService`](MultiDocPrintService.md "interface in javax.print") objects. If no
        services match, the array is zero-length.
  + ### lookupDefaultPrintService

    public static final [PrintService](PrintService.md "interface in javax.print") lookupDefaultPrintService()

    Locates the default print service for this environment. This may return
    `null`. If multiple lookup services each specify a default, the
  ...
  + ### registerServiceProvider
    ...
    public static boolean registerServiceProvider([PrintServiceLookup](PrintServiceLookup.md "class in javax.print") sp)
    ...
    Returns:
    :   `true` if the new lookup service is newly registered;
        `false` otherwise
  + ### registerService

    public static boolean registerService([PrintService](PrintService.md "interface in javax.print") service)

    Allows an application to directly register an instance of a class which
    implements a print service. The lookup operations for this service will
    be performed by the `PrintServiceLookup` class using the attribute
    ...
    `PrintServiceLookup` instance instead is recommended. The method
    returns `true` if this service is not previously registered and is
    now successfully registered. This method should not be called with
    `StreamPrintService` instances. They will always fail to register
    ...
    Returns:
    :   `true` if the service is newly registered; `false`
        otherwise
  + ### getPrintServices

    public abstract [PrintService](PrintService.md "interface in javax.print")[] getPrintServices([DocFlavor](DocFlavor.md "class in javax.print") flavor,
    [AttributeSet](attribute/AttributeSet.md "interface in javax.print.attribute") attributes)

    Locates services that can be positively confirmed to support the
    combination of attributes and `DocFlavors` specified. This method
    ...
    Returns:
    :   array of matching `PrintServices`. If no services match,
        the array is zero-length.
  + ### getPrintServices

    public abstract [PrintService](PrintService.md "interface in javax.print")[] getPrintServices()
  ...
  + ### getMultiDocPrintServices
    ...
    public abstract [MultiDocPrintService](MultiDocPrintService.md "interface in javax.print")[] getMultiDocPrintServices([DocFlavor](DocFlavor.md "class in javax.print")[] flavors,
    [AttributeSet](attribute/AttributeSet.md "interface in javax.print.attribute") attributes)
    ...
    to support the combination of attributes and `DocFlavors`
    specified.

    Parameters:
    :   `flavors` - of documents required. If `null` or empty it is
        ignored.
    :   `attributes` - required to be supported. If `null` this
        constraint is not used.
  ...
  + ### getDefaultPrintService
    ...
    public abstract [PrintService](PrintService.md "interface in javax.print") getDefaultPrintService()

    Not called directly by applications. Implemented by a service provider,
    and called by the print lookup service.

    Returns:
    :   the default `PrintService` for this lookup service. If
        there is no default, returns `null`.