...
interface [`PrintRequestAttribute`](PrintRequestAttribute.md "interface in javax.print.attribute"). The client
uses a `PrintRequestAttributeSet` to specify the settings to be applied
to a whole print job and to all the docs in the print job.

`PrintRequestAttributeSet` is just an [`AttributeSet`](AttributeSet.md "interface in javax.print.attribute")
whose constructors and mutating operations guarantee an additional invariant,
namely that all attribute values in the `PrintRequestAttributeSet` must
be instances of interface
[`PrintRequestAttribute`](PrintRequestAttribute.md "interface in javax.print.attribute"). The
...
* ## Method Details
  ...
  + ### add
    ...
    :   `ClassCastException` - if the `attribute` is not an instance of
        interface [`PrintRequestAttribute`](PrintRequestAttribute.md "interface in javax.print.attribute")
