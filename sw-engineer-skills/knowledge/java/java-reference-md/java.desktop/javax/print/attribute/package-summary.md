...
When setting up a print job, a client specifies two things: **print data**
and **processing instructions.** The print data is the actual content to
...
* You can print the same print data at different times using different
  ...
  talk.* You can use the same processing instructions at different times to
...
Service API uses the term **attribute category** to refer to a printer
capability and the term **attribute value** to refer to the value of the
capability.

In the Java Print Service API, an attribute category is represented by a Java
class implementing the [Attribute](Attribute.md) interface.
...
instance of the [Copies](standard/Copies.md) class with the
number of desired copies and uses the `Copies` instance as part of the
print request. In this case, the `Copies` class represents the
attribute category, and the `Copies` instance represents the attribute
value.
...
*completed* or *queued*) and the number of pages printed so far.
...
* [DocAttribute](DocAttribute.md) specifies a characteristic
  of an individual document and the print job settings to be applied to an
  individual document.* [PrintRequestAttribute](PrintRequestAttribute.md)
    specifies a setting applied to a whole print job and to all the documents
    in the print job.* [PrintJobAttribute](PrintJobAttribute.md) reports the
      status of a print job.* [PrintServiceAttribute](PrintServiceAttribute.md) reports
        the status of a print service.* [SupportedValuesAttribute](SupportedValuesAttribute.md)
...
in a print job as a `DocAttribute` or to an entire print job as a
`PrintRequestAttribute`. Certain low-level attributes are never used on
...
[Attribute](Attribute.md), not any of the tagging subinterfaces.
...
[`Map`](../../../../java.base/java/util/Map.md "interface in java.util") interface: it provides a map of
...
1. Each key in an `AttributeSet` corresponds to a category, and the
   value of the key can only be one of the attribute values that belong to the
   category represented by the key. Thus, unlike a `Map`, an
   `AttributeSet` restricts the possible values of a key: an attribute
   category cannot be set to an attribute value that does not belong to that
   category.- No two attributes from the same category can exist in the same set. For
     ...
     printer conflicting instructions.- Only attributes implementing the `Attribute` interface can be
       added to the set.

The `javax.print.attribute` package includes
[HashAttributeSet](HashAttributeSet.md) as a concrete
...
discussed in the [Attribute Roles](#role) section:

* [DocAttributeSet](DocAttributeSet.md)* [PrintRequestAttributeSet](PrintRequestAttributeSet.md)* [PrintJobAttributeSet](PrintJobAttributeSet.md)* [PrintServiceAttributeSet](PrintServiceAttributeSet.md)

Notice that only four kinds of attribute sets are listed here, but there are
five kinds of attributes. Interface
[SupportedValuesAttribute](SupportedValuesAttribute.md) denotes
...
Package `javax.print.attribute` includes one concrete implementation of
each of the attribute set subinterfaces:

* [HashDocAttributeSet](HashDocAttributeSet.md)* [HashPrintRequestAttributeSet](HashPrintRequestAttributeSet.md),* [HashPrintJobAttributeSet](HashPrintJobAttributeSet.md),* [HashPrintServiceAttributeSet](HashPrintServiceAttributeSet.md).

All of these classes extend
[HashAttributeSet](HashAttributeSet.md) and enforce the
...
* Primitive data types are not type-safe. For example, a compiler should
  not allow a "copies" attribute value to be used for a "sides" attribute.* Some attributes must be represented as a record of several values. One
...
`Copies`, class [Sides](standard/Sides.md), and class
[PrinterResolution](standard/PrinterResolution.md). Each
...
* [EnumSyntax](EnumSyntax.md) provides a type-safe
  ...
  that wraps a hidden int value.* [IntegerSyntax](IntegerSyntax.md) is the abstract syntax
    for integer-valued attributes.* [TextSyntax](TextSyntax.md) is the abstract syntax for
      text-valued attributes, and includes a locale giving the text string's
      natural language.* [SetOfIntegerSyntax](SetOfIntegerSyntax.md) is the
        abstract syntax for attributes representing a range or set of integers* [ResolutionSyntax](ResolutionSyntax.md) is the abstract
          syntax for attributes representing resolution values, such as 600x300
          dpi.* [Size2DSyntax](Size2DSyntax.md) is the abstract syntax for
            attributes representing a two-dimensional size, such as a paper size of
            8.5 x 11 inches.* [DateTimeSyntax](DateTimeSyntax.md) is the abstract syntax
              for attributes whose value is a date and time.* [URISyntax](URISyntax.md) is the abstract syntax for
...
implements one or more subinterfaces of `Attribute`. Most attribute
classes also extend the appropriate abstract syntax class to get the
implementation. Consider the `Sides` attribute class:
> ```
>  public class Sides
>      extends EnumSyntax
...
[`getCategory`](Attribute.md#getCategory()) method,
...
* define new vendor-specific values for any standard attribute defined in
  [javax.print.attribute.standard](standard/package-summary.md).* define new attribute categories representing the vendor printer's
...
attribute using an abstract syntax class of `EnumSyntax` specifies all
...
`Attribute` or one of its subinterfaces and extends an abstract syntax
...
`Attribute` is used, such as in an `AttributeSet`.
...
* Related Packages
  ...
  [javax.print.attribute.standard](standard/package-summary.md)

  Package `javax.print.attribute.standard` contains classes for specific
  printing attributes.

  [javax.print.event](../event/package-summary.md)
...
* All Classes and InterfacesInterfacesClassesException Classes
  ...
  [AttributeSetUtilities](AttributeSetUtilities.md "class in javax.print.attribute")

  Class `AttributeSetUtilities` provides static methods for manipulating
  `AttributeSets`.

  [DateTimeSyntax](DateTimeSyntax.md "class in javax.print.attribute")

  Class `DateTimeSyntax` is an abstract base class providing the common
  implementation of all attributes whose value is a date and time.

  [DocAttribute](DocAttribute.md "interface in javax.print.attribute")
  ...
  [EnumSyntax](EnumSyntax.md "class in javax.print.attribute")

  Class `EnumSyntax` is an abstract base class providing the common
  implementation of all "type safe enumeration" objects.

  [HashAttributeSet](HashAttributeSet.md "class in javax.print.attribute")
  ...
  [HashPrintJobAttributeSet](HashPrintJobAttributeSet.md "class in javax.print.attribute")
  ...
  [IntegerSyntax](IntegerSyntax.md "class in javax.print.attribute")

  Class `IntegerSyntax` is an abstract base class providing the common
  implementation of all attributes with integer values.

  [PrintJobAttribute](PrintJobAttribute.md "interface in javax.print.attribute")
  ...
  of print request attributes, i.e.
  ...
  of print job attributes, i.e.

  [ResolutionSyntax](ResolutionSyntax.md "class in javax.print.attribute")
  ...
  [SetOfIntegerSyntax](SetOfIntegerSyntax.md "class in javax.print.attribute")
  ...
  [Size2DSyntax](Size2DSyntax.md "class in javax.print.attribute")
