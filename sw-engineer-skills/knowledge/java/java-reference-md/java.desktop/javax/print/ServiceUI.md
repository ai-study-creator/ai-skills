...
of the `AttributeSet`. Applications which want to preserve the user
...
* ## Method Details
  ...
  + ### printDialog
    ...
    public static [PrintService](PrintService.md "interface in javax.print") printDialog([GraphicsConfiguration](../../java/awt/GraphicsConfiguration.md "class in java.awt") gc,
    int x,
    int y,
    [PrintService](PrintService.md "interface in javax.print")[] services,
    [PrintService](PrintService.md "interface in javax.print") defaultService,
    ...
    the `PrintService`'s `ServiceUIFactory` when the
    `PrintService` is browsed. The dialog will attempt to locate a
    `MAIN_UIROLE` first as a `JComponent`, then as a
    `Panel`. If there is no `ServiceUIFactory` or no matching
    ...
    array must be `non-null` and non-empty. Typically an application
    will pass in only `PrintServices` capable of printing a particular
    document flavor.

    An application may pass in a `PrintService` to be initially
    displayed. A `non-null` parameter must be included in the array of
    browsable services. If this parameter is `null` a service is chosen
    by the implementation.

    An application may optionally pass in the flavor to be printed. If this
    is `non-null` choices presented to the user can be better validated
    against those supported by the services. An application must pass in a
    `PrintRequestAttributeSet` for returning user choices. On calling
    the `PrintRequestAttributeSet` may be empty, or may contain
    ...
    Parameters:
    :   `gc` - used to select screen, `null` means primary or default
        screen
    :   `x` - location of dialog including border in screen coordinates
        relative to the origin of `gc`
    ...
    :   `attributes` - on input is the initial application supplied
        preferences. This cannot be `null` but may be empty. On
    ...
    :   `HeadlessException` - if `GraphicsEnvironment.isHeadless()`
        returns `true`
    :   `IllegalArgumentException` - if services is `null` or empty, or
        attributes is `null`, or the initial `PrintService`
