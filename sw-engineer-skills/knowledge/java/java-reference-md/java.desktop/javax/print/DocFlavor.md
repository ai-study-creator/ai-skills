...
which print data is supplied to a [`DocPrintJob`](DocPrintJob.md "interface in javax.print"). "Doc" is a short,
...
* **MIME type.** This is a Multipurpose Internet Mail Extensions
  ...
  [RFC 2046](http://www.ietf.org/rfc/rfc2046.txt)) that specifies
  ...
  [character encodings](../../../java.base/java/lang/package-summary.md#charenc) for more information on the character encodings
  supported on the Java platform.* **Representation class name.** This specifies the fully-qualified
    name of the class of the object from which the actual print data comes, as
    returned by the [`Class.getName()`](../../../java.base/java/lang/Class.md#getName()) method. (Thus the
    class name for `byte[]` is `"[B"`, for `char[]` it is
    `"[C"`.)

A `DocPrintJob` obtains its print data by means of interface
[`Doc`](Doc.md "interface in javax.print"). A `Doc` object lets the `DocPrintJob` determine
...
[Media Types Directory](http://www.iana.org/assignments/media-types/). Interface [`Doc`](Doc.md "interface in javax.print") provides two utility operations,
[`getReaderForText`](Doc.md#getReaderForText()) and
[`getStreamForBytes()`](Doc.md#getStreamForBytes()), to help a `Doc`
...
* Character array (`char[]`) -- The print data consists of the
  Unicode characters in the array.* `String` -- The print data consists of the Unicode characters in
    the string.* Character stream ([`java.io.Reader`](../../../java.base/java/io/Reader.md "class in java.io")) -- The print
      data consists of the Unicode characters read from the stream up to the
      end-of-stream.* Byte array (`byte[]`) -- The print data consists of the bytes in
        ...
        default character set is US-ASCII.* Byte stream ([`java.io.InputStream`](../../../java.base/java/io/InputStream.md "class in java.io")) -- The
          ...
          the default character set is US-ASCII.* Uniform Resource Locator ([`URL`](../../../java.base/java/net/URL.md "class in java.net")) -- The print data
...
[RFC 2046](http://www.ietf.org/rfc/rfc2046.txt), which says the
...
[`DocFlavor.hostEncoding`](#hostEncoding) This may not always be
...
See [character encodings](../../../java.base/java/lang/package-summary.md#charenc) for more information on the character encodings
...
* Preformatted text
  ...
  | --- | --- |
  | `"text/plain"` Plain text in the default character set (US-ASCII)|  |  |  |  |  |  | | --- | --- | --- | --- | --- | --- | | `"text/plain; charset=xxx"` Plain text in character set *xxx*| `"text/html"` HyperText Markup Language in the default character set (US-ASCII)|  |  | | --- | --- | | `"text/html; charset=xxx"` HyperText Markup Language in character set *xxx* | | | | | | | |
  ...
  byte oriented representation class (byte array, InputStream, URL).* Preformatted page description language (PDL) documents

    MIME-Types and their descriptions

    | MIME-Type Description | |
    | --- | --- |
    | `"application/pdf"` Portable Document Format document|  |  |  |  | | --- | --- | --- | --- | | `"application/postscript"` PostScript document|  |  | | --- | --- | | `"application/vnd.hp-PCL"` Printer Control Language document | | | | | |

    In general, preformatted PDL print data is provided in a byte oriented
    representation class (byte array, `InputStream`, `URL`).* Preformatted images

      MIME-Types and their descriptions

      | MIME-Type Description | |
      | --- | --- |
      | `"image/gif"` Graphics Interchange Format image|  |  |  |  | | --- | --- | --- | --- | | `"image/jpeg"` Joint Photographic Experts Group image|  |  | | --- | --- | | `"image/png"` Portable Network Graphics image | | | | | |

      In general, preformatted image print data is provided in a byte oriented
      representation class (byte array, `InputStream`, `URL`).* Preformatted autosense print data
...
* Renderable image object -- The client supplies an object that
  ...
  [`RenderableImage`](../../java/awt/image/renderable/RenderableImage.md "interface in java.awt.image.renderable"). The
  printer calls methods in that interface to obtain the image to be printed.* Printable object -- The client supplies an object that implements
    interface [`Printable`](../../java/awt/print/Printable.md "interface in java.awt.print"). The printer calls
    ...
    client draws in that graphics context gets printed.* Pageable object -- The client supplies an object that implements
      interface [`Pageable`](../../java/awt/print/Pageable.md "interface in java.awt.print"). The printer calls
      methods in that interface to obtain the pages to be printed, one by one.
      For each page, the printer supplies a graphics context, and whatever the
...
* Plain text print data provided through a byte stream. Specifically, the
  ...
  ·  
  `("text/plain; charset=utf-8", "java.io.InputStream")`* Renderable image objects. Specifically, the following doc flavor is
...
* The character pair carriage return-line feed (CR-LF) means "go to
  column 1 of the next line."* A carriage return (CR) character standing by itself means "go to column
    1 of the next line."* A line feed (LF) character standing by itself means "go to column 1 of
      the next line."
...
[`DataFlavor`](../../../java.datatransfer/java/awt/datatransfer/DataFlavor.md "class in java.awt.datatransfer"). Class `DataFlavor` is not
...
1. The JPS API is designed to be used in Java profiles which do not
   support AWT.- The implementation of class `java.awt.datatransfer.DataFlavor`
     ...
     services which need this.- The implementation of class `java.awt.datatransfer.DataFlavor`
...
* The media type, media subtype, and parameters are retained, but all
  comments and whitespace characters are discarded.* The media type, media subtype, and parameter names are converted to
    lowercase.* The parameter values retain their original case, except a charset
      parameter value for a text media type is converted to lowercase.* Quote characters surrounding parameter values are removed.* Quoting backslash characters inside parameter values are removed.* The parameters are arranged in ascending order of parameter name.
...
See Also:
:   * [Serialized Form](../../../serialized-form.md#javax.print.DocFlavor)
...
* ## Field Details
  ...
  + ### hostEncoding
    ...
    public static final [String](../../../java.base/java/lang/String.md "class in java.lang") hostEncoding

    A string representing the host operating system encoding. This will
    follow the conventions documented in
    [*RFC 2278: IANA Charset Registration Procedures*](http://www.ietf.org/rfc/rfc2278.txt)
...
* ## Constructor Details
  ...
  + ### DocFlavor
    ...
    public DocFlavor([String](../../../java.base/java/lang/String.md "class in java.lang") mimeType,
    ...
    Throws:
    :   `NullPointerException` - if `mimeType` or `className` is
        `null`
    :   `IllegalArgumentException` - if `mimeType` does not obey the
...
* ## Method Details
  ...
  + ### getMediaType
    ...
    public [String](../../../java.base/java/lang/String.md "class in java.lang") getMediaType()
  ...
  + ### getMediaSubtype
    ...
    public [String](../../../java.base/java/lang/String.md "class in java.lang") getMediaSubtype()
  ...
  + ### getParameter
    ...
    Returns a `String` representing a MIME parameter. Mime types may
    ...
    Parameters:
    :   `paramName` - the name of the parameter. This name is internally
        converted to the canonical lower case format before performing
        the match.

    Returns:
    :   a string representing a mime parameter, or `null` if that
  ...
  + ### getRepresentationClassName
    ...
    public [String](../../../java.base/java/lang/String.md "class in java.lang") getRepresentationClassName()
