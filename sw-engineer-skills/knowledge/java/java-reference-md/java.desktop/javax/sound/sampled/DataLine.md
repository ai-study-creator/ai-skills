...
subinterfaces [`SourceDataLine`](SourceDataLine.md "interface in javax.sound.sampled") or [`Clip`](Clip.md "interface in javax.sound.sampled"), which allow an
application program to write data. Similarly, audio input is handled by the
subinterface [`TargetDataLine`](TargetDataLine.md "interface in javax.sound.sampled"), which allows data to be read.

A data line has an internal buffer in which the incoming or outgoing audio
data is queued. The [`drain()`](#drain()) method blocks until this internal buffer
becomes empty, usually because all queued data has been processed. The
[`flush()`](#flush()) method discards any available queued data from the internal
buffer.

A data line produces [`START`](LineEvent.Type.md#START) and
[`STOP`](LineEvent.Type.md#STOP) events whenever it begins or ceases active
...
if [`start()`](#start()) is called on an inactive data line, and data is available
...
[`synchronize`](Mixer.md#synchronize(javax.sound.sampled.Line%5B%5D,boolean)) method. See the description of the
[`Mixer`](Mixer.md "interface in javax.sound.sampled") interface for a more complete description.
...
* ## Method Details
  ...
  + ### start
    ...
    playback starts, a [`START`](LineEvent.Type.md#START) event is generated.
  ...
  + ### stop
    ...
    a [`STOP`](LineEvent.Type.md#STOP) event is generated.
  ...
  + ### isRunning
    ...
    Indicates whether the line is running. The default is `false`. An
  ...
  + ### isActive
    ...
    [`START`](LineEvent.Type.md#START) event to its listeners. Similarly,
    when an active line becomes inactive, it sends a
    [`STOP`](LineEvent.Type.md#STOP) event.
  ...
  + ### getFormat
    ...
    [AudioFormat](AudioFormat.md "class in javax.sound.sampled") getFormat()
    ...
    specific audio format (e.g. [`SourceDataLine.open(AudioFormat)`](SourceDataLine.md#open(javax.sound.sampled.AudioFormat)))
