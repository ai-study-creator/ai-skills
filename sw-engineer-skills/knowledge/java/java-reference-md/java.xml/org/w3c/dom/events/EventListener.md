...
method the `EventListener`s attached to the source
`Node` are not attached to the copied `Node`. If
the user wishes the same `EventListener`s to be added to the
...
* ## Method Details
  ...
  + ### handleEvent
    ...
    void handleEvent([Event](Event.md "interface in org.w3c.dom.events") evt)
