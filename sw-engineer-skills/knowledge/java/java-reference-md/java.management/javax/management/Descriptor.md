...
extends [Serializable](../../../java.base/java/io/Serializable.md "interface in java.io"), [Cloneable](../../../java.base/java/lang/Cloneable.md "interface in java.lang")

Additional metadata for a JMX element. A `Descriptor`
is associated with a [`MBeanInfo`](MBeanInfo.md "class in javax.management"), [`MBeanAttributeInfo`](MBeanAttributeInfo.md "class in javax.management"), etc.
...
in the result of the [`getFields()`](#getFields()) and [`getFieldNames()`](#getFieldNames())
methods.
...
instances of [`ImmutableDescriptor`](ImmutableDescriptor.md "class in javax.management") or a subclass. Mutable
descriptors are usually instances of
[`DescriptorSupport`](modelmbean/DescriptorSupport.md "class in javax.management.modelmbean") or a subclass.
...
below, and each one has a corresponding constant in the [`JMX`](JMX.md "class in javax.management")
class. For example, the field `defaultValue` is represented
by the constant [`JMX.DEFAULT_VALUE_FIELD`](JMX.md#DEFAULT_VALUE_FIELD).
...
of the mapped Java type, called *opendata*(J) in the [MXBean type mapping rules](MXBean.md#mapping-rules).

Descriptor Fields

| Name | Type | Used in | Meaning |
| --- | --- | --- | --- |
| *defaultValue* Object | MBeanAttributeInfo MBeanParameterInfo | Default value for an attribute or parameter. See [`javax.management.openmbean`](openmbean/package-summary.md). || deprecated | String | Any | An indication that this element of the information model is no longer recommended for use. A set of MBeans defined by an application is collectively called an *information model*. The convention is for the value of this field to contain a string that is the version of the model in which the element was first deprecated, followed by a space, followed by an explanation of the deprecation, for example `"1.3 Replaced by the Capacity attribute"`. || descriptionResource  BundleBaseName | String | Any | The base name for the [`ResourceBundle`](../../../java.base/java/util/ResourceBundle.md "class in java.util") in which the key given in the `descriptionResourceKey` field can be found, for example `"com.example.myapp.MBeanResources"`. The meaning of this field is defined by this specification but the field is not set or used by the JMX API itself. || descriptionResourceKey | String | Any | A resource key for the description of this element. In conjunction with the `descriptionResourceBundleBaseName`, this can be used to find a localized version of the description. The meaning of this field is defined by this specification but the field is not set or used by the JMX API itself. || enabled | String | MBeanAttributeInfo MBeanNotificationInfo MBeanOperationInfo | The string `"true"` or `"false"` according as this item is enabled. When an attribute or operation is not enabled, it exists but cannot currently be accessed. A user interface might present it as a greyed-out item. For example, an attribute might only be meaningful after the `start()` method of an MBean has been called, and is otherwise disabled. Likewise, a notification might be disabled if it cannot currently be emitted but could be in other circumstances. || exceptions String[] | MBeanAttributeInfo, MBeanConstructorInfo, MBeanOperationInfo | The class names of the exceptions that can be thrown when invoking a constructor or operation, or getting an attribute. The meaning of this field is defined by this specification but the field is not set or used by the JMX API itself. Exceptions thrown when setting an attribute are specified by the field [`setExceptions`](#setExceptions).| *immutableInfo* String | MBeanInfo | The string `"true"` or `"false"` according as this MBean's MBeanInfo is *immutable*. When this field is true, the MBeanInfo for the given MBean is guaranteed not to change over the lifetime of the MBean. Hence, a client can read it once and cache the read value. When this field is false or absent, there is no such guarantee, although that does not mean that the MBeanInfo will necessarily change. See also the [`"jmx.mbean.info.changed"`](MBeanInfo.md#info-changed) notification. || infoTimeout | String Long | MBeanInfo | The time in milli-seconds that the MBeanInfo can reasonably be expected to be unchanged. The value can be a `Long` or a decimal string. This provides a hint from a DynamicMBean or any MBean that does not define `immutableInfo` as `true` that the MBeanInfo is not likely to change within this period and therefore can be cached. When this field is missing or has the value zero, it is not recommended to cache the MBeanInfo unless it has the `immutableInfo` set to `true` or it has [`"jmx.mbean.info.changed"`](MBeanInfo.md#info-changed) in its [`MBeanNotificationInfo`](MBeanNotificationInfo.md "class in javax.management") array. | | *interfaceClassName* | String | MBeanInfo | The Java interface name for a Standard MBean or MXBean, as returned by [`Class.getName()`](../../../java.base/java/lang/Class.md#getName()). A Standard MBean or MXBean registered directly in the MBean Server or created using the [`StandardMBean`](StandardMBean.md "class in javax.management") class will have this field in its MBeanInfo Descriptor. || *legalValues* | Set<?> | MBeanAttributeInfo MBeanParameterInfo | Legal values for an attribute or parameter. See [`javax.management.openmbean`](openmbean/package-summary.md). || locale | String | Any | The [locale](../../../java.base/java/util/Locale.md "class in java.util") of the description in this `MBeanInfo`, `MBeanAttributeInfo`, etc, as returned by [`Locale.toString()`](../../../java.base/java/util/Locale.md#toString()). || *maxValue* Object | MBeanAttributeInfo MBeanParameterInfo | Maximum legal value for an attribute or parameter. See [`javax.management.openmbean`](openmbean/package-summary.md). || metricType | String | MBeanAttributeInfo MBeanOperationInfo | The type of a metric, one of the strings "counter" or "gauge". A metric is a measurement exported by an MBean, usually an attribute but sometimes the result of an operation. A metric that is a *counter* has a value that never decreases except by being reset to a starting value. Counter metrics are almost always non-negative integers. An example might be the number of requests received. A metric that is a *gauge* has a numeric value that can increase or decrease. Examples might be the number of open connections or a cache hit rate or a temperature reading.| *minValue* Object | MBeanAttributeInfo MBeanParameterInfo | Minimum legal value for an attribute or parameter. See [`javax.management.openmbean`](openmbean/package-summary.md). || *mxbean* String | MBeanInfo | The string `"true"` or `"false"` according as this MBean is an [`MXBean`](MXBean.md "annotation interface in javax.management"). A Standard MBean or MXBean registered directly with the MBean Server or created using the [`StandardMBean`](StandardMBean.md "class in javax.management") class will have this field in its MBeanInfo Descriptor. || *openType* [`OpenType`](openmbean/OpenType.md "class in javax.management.openmbean") | MBeanAttributeInfo MBeanOperationInfo MBeanParameterInfo | The Open Type of this element. In the case of `MBeanAttributeInfo` and `MBeanParameterInfo`, this is the Open Type of the attribute or parameter. In the case of `MBeanOperationInfo`, it is the Open Type of the return value. This field is set in the Descriptor for all instances of [`OpenMBeanAttributeInfoSupport`](openmbean/OpenMBeanAttributeInfoSupport.md "class in javax.management.openmbean"), [`OpenMBeanOperationInfoSupport`](openmbean/OpenMBeanOperationInfoSupport.md "class in javax.management.openmbean"), and [`OpenMBeanParameterInfoSupport`](openmbean/OpenMBeanParameterInfoSupport.md "class in javax.management.openmbean"). It is also set for attributes, operations, and parameters of MXBeans.  This field can be set for an `MBeanNotificationInfo`, in which case it indicates the Open Type that the [`user data`](Notification.md#getUserData()) will have. || *originalType* String | MBeanAttributeInfo MBeanOperationInfo MBeanParameterInfo | The original Java type of this element as it appeared in the [`MXBean`](MXBean.md "annotation interface in javax.management") interface method that produced this `MBeanAttributeInfo` (etc). For example, a method  `public` [`MemoryUsage`](../../java/lang/management/MemoryUsage.md "class in java.lang.management") `getHeapMemoryUsage();`  in an MXBean interface defines an attribute called `HeapMemoryUsage` of type [`CompositeData`](openmbean/CompositeData.md "interface in javax.management.openmbean"). The `originalType` field in the Descriptor for this attribute will have the value `"java.lang.management.MemoryUsage"`. The format of this string is described in the section [Type Names](MXBean.md#type-names) of the MXBean specification. | *setExceptions* String[] | MBeanAttributeInfo | The class names of the exceptions that can be thrown when setting an attribute. The meaning of this field is defined by this specification but the field is not set or used by the JMX API itself. Exceptions thrown when getting an attribute are specified by the field [`exceptions`](#exceptions).| severity | String Integer | MBeanNotificationInfo | The severity of this notification. It can be 0 to mean unknown severity or a value from 1 to 6 representing decreasing levels of severity. It can be represented as a decimal string or an `Integer`. || since | String | Any | The version of the information model in which this element was introduced. A set of MBeans defined by an application is collectively called an *information model*. The application may also define versions of this model, and use the `"since"` field to record the version in which an element first appeared. || units | String | MBeanAttributeInfo MBeanParameterInfo MBeanOperationInfo | The units in which an attribute, parameter, or operation return value is measured, for example `"bytes"` or `"seconds"`. | | | | | | | | | | | | | |

Some additional fields are defined by Model MBeans. See the
information for [`ModelMBeanInfo`](modelmbean/ModelMBeanInfo.md#descriptor),
[`ModelMBeanAttributeInfo`](modelmbean/ModelMBeanAttributeInfo.md#descriptor),
[`ModelMBeanConstructorInfo`](modelmbean/ModelMBeanConstructorInfo.md#descriptor),
[`ModelMBeanNotificationInfo`](modelmbean/ModelMBeanNotificationInfo.md#descriptor), and
[`ModelMBeanOperationInfo`](modelmbean/ModelMBeanOperationInfo.md#descriptor), as
well as the chapter "Model MBeans" of the [JMX
Specification](http://www.oracle.com/technetwork/java/javase/tech/javamanagement-140525.html). The following table summarizes these fields. Note
...
MBeans. The [displayName](#displayName), [severity](#severity), and [visibility](#visibility) fields are of
...
| currencyTimeLimit | Number | ModelMBeanInfo ModelMBeanAttributeInfo ModelMBeanOperationInfo | How long cached value is valid: <0 never, =0 always, >0 seconds. |
...
| lastUpdatedTimeStamp | Number | ModelMBeanAttributeInfo ModelMBeanOperationInfo | When [value](#value-field) was set. |
| log | String | ModelMBeanInfo ModelMBeanNotificationInfo | t or T: log all notifications, f or F: log no notifications. |
| logFile | String | ModelMBeanInfo ModelMBeanNotificationInfo | Fully qualified filename to log events to. |
...
| persistPeriod | Number | ModelMBeanInfo ModelMBeanAttributeInfo | Frequency of persist cycle in seconds. Used when persistPolicy is "OnTimer" or "NoMoreOftenThan". |
| persistPolicy | String | ModelMBeanInfo ModelMBeanAttributeInfo | One of: OnUpdate|OnTimer|NoMoreOftenThan|OnUnregister|Always|Never. See the section "MBean Descriptor Fields" in the JMX specification document. |
...
| role | String | ModelMBeanConstructorInfo ModelMBeanOperationInfo | One of "constructor", "operation", "getter", or "setter". |
...
| value | Object | ModelMBeanAttributeInfo ModelMBeanOperationInfo | Current (cached) value for attribute or operation. |
...
* ## Method Details
  ...
  + ### getFieldValue
    ...
    [Object](../../../java.base/java/lang/Object.md "class in java.lang") getFieldValue([String](../../../java.base/java/lang/String.md "class in java.lang") fieldName)
  ...
  + ### setField
    ...
    void setField([String](../../../java.base/java/lang/String.md "class in java.lang") fieldName,
  ...
  + ### getFields
    ...
    [String](../../../java.base/java/lang/String.md "class in java.lang")[] getFields()
    ...
    See Also:
    :   - [`setFields(java.lang.String[], java.lang.Object[])`](#setFields(java.lang.String%5B%5D,java.lang.Object%5B%5D))
  + ### getFieldNames

    [String](../../../java.base/java/lang/String.md "class in java.lang")[] getFieldNames()
  ...
  + ### getFieldValues
    ...
    [Object](../../../java.base/java/lang/Object.md "class in java.lang")[] getFieldValues([String](../../../java.base/java/lang/String.md "class in java.lang")... fieldNames)
    ...
    :   `fieldNames` - String array of the names of the fields that
        ...
        returned by [`getFieldNames()`](#getFieldNames()). If a field name in the
  ...
  + ### removeField
    ...
    void removeField([String](../../../java.base/java/lang/String.md "class in java.lang") fieldName)
  ...
  + ### setFields
    ...
    void setFields([String](../../../java.base/java/lang/String.md "class in java.lang")[] fieldNames,
  ...
  + ### isValid
    ...
    Throws:
    :   `RuntimeOperationsException` - If the validity checking fails for
