...
by [`javax.management.openmbean`](openmbean/package-summary.md). In this way, you can be
...
`Usage` of type [`MemoryUsage`](../../java/lang/management/MemoryUsage.md "class in java.lang.management"). The point of an
...
[`getAttribute`](MBeanServer.md#getAttribute(javax.management.ObjectName,java.lang.String)) calls, then we could
...
the other hand, we have added a [`@ConstructorParameters`](ConstructorParameters.md "annotation interface in javax.management")
...
[`CompositeData`](openmbean/CompositeData.md "interface in javax.management.openmbean") and
[`TabularData`](openmbean/TabularData.md "interface in javax.management.openmbean") classes
from the standard [`javax.management.openmbean`](openmbean/package-summary.md) package, it
...
argument to [`StandardMBean`](StandardMBean.md "class in javax.management") and the resultant MBean
registered in the MBean Server.

When an object is registered in the MBean Server using the
`registerMBean` or `createMBean` methods of the
[`MBeanServer`](MBeanServer.md "interface in javax.management") interface, the object's class is examined
to determine what type of MBean it is:

* If the class implements the interface [`DynamicMBean`](DynamicMBean.md "interface in javax.management")
...
* Otherwise, it may be an MXBean. The set of interfaces
  ...
  + have an annotation `@MXBean(true)`
    or just `@MXBean`.If there is exactly one such interface, or if there is one
  ...
  *MXBean interface*. In the example above, the MXBean
  interface is `MemoryPoolMXBean`.* If none of these conditions is met, the MBean is invalid and
    the attempt to register it will generate [`NotCompliantMBeanException`](NotCompliantMBeanException.md "class in javax.management").
...
3. A method `void setN(T x)`
   ...
   irrelevant.)
4. Every other method specifies that there is an operation with
...
An MXBean is a kind of Open MBean, as defined by the [`javax.management.openmbean`](openmbean/package-summary.md) package. This means that the types of
attributes, operation parameters, and operation return values must
all be describable using *Open Types*, that is the four
standard subclasses of [`OpenType`](openmbean/OpenType.md "class in javax.management.openmbean").
...
* The corresponding Open Type, *opentype(J)*. This is
  an instance of a subclass of [`OpenType`](openmbean/OpenType.md "class in javax.management.openmbean").
...
* The Open Type, *opentype(*`List<String>`*)*, is [`ArrayType`](openmbean/ArrayType.md "class in javax.management.openmbean")`(1,` [`SimpleType.STRING`](openmbean/SimpleType.md#STRING)`)`, representing a 1-dimensional
  array of `String`s.
* The mapped Java type, *opendata(*`List<String>`*)*, is `String[]`.
* A `List<String>` can be converted to a `String[]`
  using [`List.toArray(new
  String[0])`](../../../java.base/java/util/List.md#toArray(T%5B%5D)).
* A `String[]` can be converted to a `List<String>`
  using [`Arrays.asList`](../../../java.base/java/util/Arrays.md#asList(T...)).
...
generated by [`JMX.newMXBeanProxy`](JMX.md#newMXBeanProxy(javax.management.MBeanServerConnection,javax.management.ObjectName,java.lang.Class)), it is the return values
...
| `Integer`, `ObjectName`, etc  (the types covered by [`SimpleType`](openmbean/SimpleType.md "class in javax.management.openmbean")) | the corresponding `SimpleType` | *J*, the same type |
| `int[]` etc  (a one-dimensional array with primitive element type) | `ArrayType.getPrimitiveArrayType(int[].class)` etc | *J*, the same type || *E*`[]`  (an array with non-primitive element type *E*; this includes `int[][]`, where *E* is `int[]`) | `ArrayType.getArrayType(`*opentype(E)*`)` | *opendata(E)*`[]` |
...
| `Map<`*K*,*V*`>`  `SortedMap<`*K*,*V*`>` | [`TabularType`](openmbean/TabularType.md "class in javax.management.openmbean")  (see below) | [`TabularData`](openmbean/TabularData.md "interface in javax.management.openmbean")  (see below) |
| [Record classes](../../../java.base/java/lang/Record.md "class in java.lang") | [`CompositeType`](openmbean/CompositeType.md "class in javax.management.openmbean"), if possible  (see below) | [`CompositeData`](openmbean/CompositeData.md "interface in javax.management.openmbean")  (see below) |
| An MXBean interface | `SimpleType.OBJECTNAME`  (see below) | [`ObjectName`](ObjectName.md "class in javax.management")  (see below) |
| Any other type | [`CompositeType`](openmbean/CompositeType.md "class in javax.management.openmbean"), if possible  (see below) | [`CompositeData`](openmbean/CompositeData.md "interface in javax.management.openmbean")  (see below) |
...
*E* is a class or interface that implements [`Comparable`](../../../java.base/java/lang/Comparable.md "interface in java.lang"). Thus, a `SortedSet<String>` or
...
non-null [`comparator()`](../../../java.base/java/util/SortedSet.md#comparator()).
...
A `Map<`*K*,*V*`>` or `SortedMap<`*K*,*V*`>`, for example `Map<String,ObjectName>`, has Open Type [`TabularType`](openmbean/TabularType.md "class in javax.management.openmbean") and is mapped
to a [`TabularData`](openmbean/TabularData.md "interface in javax.management.openmbean").
...
The `typeName` here is determined by the [type name rules](#type-names) detailed below.

A `SortedMap<`*K*,*V*`>` is mapped in the
same way, but it is only convertible if
*K* is a class or interface that implements [`Comparable`](../../../java.base/java/lang/Comparable.md "interface in java.lang"). Thus, a `SortedMap<String,int[]>`
is convertible, but a
`SortedMap<int[],String>` is not. The conversion of a
`SortedMap` instance will fail with an `IllegalArgumentException` if it has a non-null [`comparator()`](../../../java.base/java/util/SortedMap.md#comparator()).
...
Open Data is [`TabularDataSupport`](openmbean/TabularDataSupport.md "class in javax.management.openmbean"),
or another class implementing `TabularData` that serializes as `TabularDataSupport`.

### Mappings for Records

A [record](../../../java.base/java/lang/Record.md "class in java.lang") class *J* can be converted
to a [`CompositeType`](openmbean/CompositeType.md "class in javax.management.openmbean") if and only if all its
[components](../../../java.base/java/lang/Class.md#getRecordComponents()) are
convertible to open types. Otherwise, it is not convertible.
A record that has no components is not convertible.

#### Mapping a record class to `CompositeType`

A record whose components are all convertible to open
types, is itself convertible to a [`CompositeType`](openmbean/CompositeType.md "class in javax.management.openmbean").
The record class is converted to a `CompositeType`
as follows.

* The type name of the `CompositeType` is the name
  of the record class.
* The record getters are the accessors for the
  [record components](../../../java.base/java/lang/reflect/RecordComponent.md "class in java.lang.reflect").
* For each record component of type *T*, the item in
  the `CompositeType` has the same name as the record
  component and its type is *opentype(T)*, as
  defined by the [type mapping rules](#mapping-rules)
  above.

#### Mapping an instance of a record class to `CompositeData`

The mapping from an instance of a record class to a
[`CompositeData`](openmbean/CompositeData.md "interface in javax.management.openmbean") corresponding to the `CompositeType`
is the same as specified for
[other types](#composite-data-map).

#### Reconstructing an instance of a record class from a `CompositeData`

A record is reconstructed using its canonical constructor.
The canonical constructor doesn't require the presence of
[`@javax.management.ConstructorParameters`](ConstructorParameters.md "annotation interface in javax.management")
or `@java.beans.ConstructorProperties` annotations. If these
annotations are present on the canonical constructor they
will be ignored.

How an instance of a record class *J* is reconstructed
from a [`CompositeData`](openmbean/CompositeData.md "interface in javax.management.openmbean") is detailed in
[Reconstructing an instance
of Java type or record class *J* from a `CompositeData`](#reconstructing)
below.
...
interfaceX)`](JMX.md#newMXBeanProxy(javax.management.MBeanServerConnection,javax.management.ObjectName,java.lang.Class)) needs to map `objectNameY` back into `interfaceY`, another MXBean interface, it does so with `JMX.newMXBeanProxy(mbeanServerConnection, objectNameY,
...
been created by [`JMX.newMXBeanProxy`](JMX.md#newMXBeanProxy(javax.management.MBeanServerConnection,javax.management.ObjectName,java.lang.Class)); or it can have been created using [`Proxy`](../../../java.base/java/lang/reflect/Proxy.md "class in java.lang.reflect") with an invocation handler that
is [`MBeanServerInvocationHandler`](MBeanServerInvocationHandler.md "class in javax.management") or a subclass.

If the same MXBean were registered under two different
`ObjectName`s, a reference to that MXBean from another
...
result is an [`InstanceAlreadyExistsException`](InstanceAlreadyExistsException.md "class in javax.management"). Registering
...
[`NotificationBroadcaster`](NotificationBroadcaster.md "interface in javax.management")s.
...
it to a [`CompositeType`](openmbean/CompositeType.md "class in javax.management.openmbean") as follows. The type name of this
`CompositeType` is determined by the [type name rules](#type-names) below.

#### Mapping a Java type *J* to [`CompositeType`](openmbean/CompositeType.md "class in javax.management.openmbean")

The class is examined for getters using the conventions
[above](#naming-conv). (Getters must be public
...
#### Mapping from an instance of Java type or record class *J* to `CompositeData`

When the Open Type is `CompositeType`, the corresponding
mapped Java type (*opendata(J)*) is [`CompositeData`](openmbean/CompositeData.md "interface in javax.management.openmbean"). The mapping from an instance of *J* to a
`CompositeData` corresponding to the `CompositeType`
just described is done as follows. First, if *J*
implements the interface [`CompositeDataView`](openmbean/CompositeDataView.md "interface in javax.management.openmbean"), then that
interface's [`toCompositeData`](openmbean/CompositeDataView.md#toCompositeData(javax.management.openmbean.CompositeType)) method is called to do the conversion.
...
> `List<String> getNames()` (or `List<String> names()` for a record)
...
used to represent a type as Open Data is [`CompositeDataSupport`](openmbean/CompositeDataSupport.md "class in javax.management.openmbean"), or another class implementing `CompositeData` that serializes as `CompositeDataSupport`.
...
1. If *J* has a method  
   ...
   *J*.
2. Otherwise, if *J* is a [`Record`](../../../java.base/java/lang/Record.md "class in java.lang") class,
   and the record canonical constructor is applicable,
   an instance of *J* is reconstructed by calling
   ...
   `CompositeData`. The canonical constructor
   is *applicable* if all the properties named
   by the record components are present in the
   `CompositeData`.
3. Otherwise, if *J* has at least one public
   constructor with either [`@javax.management.ConstructorParameters`](ConstructorParameters.md "annotation interface in javax.management") or
   `@java.beans.ConstructorProperties` annotation, then one of those
...
4. Otherwise, if *J* has a public no-arg constructor, and
...
5. Otherwise, if *J* is an interface that has no methods
   other than getters, an instance of *J* is constructed
   using a [`Proxy`](../../../java.base/java/lang/reflect/Proxy.md "class in java.lang.reflect") with a [`CompositeDataInvocationHandler`](openmbean/CompositeDataInvocationHandler.md "class in javax.management.openmbean") backed by the `CompositeData` being converted.
...
2. Record:
   ...
   > ```
3. Public constructor with `@ConstructorParameters` annotation:
   > ```
   > public class NamedNumber {
   >     public int getNumber() {return number;}
   >     public String getName() {return name;}
...
interfaces. This is a consequence of the immutability of [`CompositeType`](openmbean/CompositeType.md "class in javax.management.openmbean"). For example, the following type could not be the
...
reasons, its [`MBeanInfo`](MBeanInfo.md "class in javax.management") is not an [`OpenMBeanInfo`](openmbean/OpenMBeanInfo.md "interface in javax.management.openmbean").
...
[`MBeanAttributeInfo`](MBeanAttributeInfo.md "class in javax.management"), [`MBeanParameterInfo`](MBeanParameterInfo.md "class in javax.management"), or
[`MBeanOperationInfo`](MBeanOperationInfo.md "class in javax.management") whose `getType()` or `getReturnType()` returns the primitive name ("`int`" etc).
This is so even though the mapping rules above specify that the
*opendata* mapping is the wrapped type (`Integer`
etc).

The array of public constructors returned by [`MBeanInfo.getConstructors()`](MBeanInfo.md#getConstructors()) for an MXBean that is directly
...
[`StandardMBean`](StandardMBean.md "class in javax.management") class is derived in the same way as for
...
The array of notification types returned by [`MBeanInfo.getNotifications()`](MBeanInfo.md#getNotifications()) for an MXBean that is directly
registered in the MBean Server will be empty if the MXBean does
not implement the [`NotificationBroadcaster`](NotificationBroadcaster.md "interface in javax.management") interface.
Otherwise, it will be the result of calling [`NotificationBroadcaster.getNotificationInfo()`](NotificationBroadcaster.md#getNotificationInfo()) at the time the MXBean
...
using the [`StandardMBean`](StandardMBean.md "class in javax.management") or [`StandardEmitterMBean`](StandardEmitterMBean.md "class in javax.management")
class is derived in the same way as for Standard MBeans.

The [`Descriptor`](Descriptor.md "interface in javax.management") for all of the
`MBeanAttributeInfo`, `MBeanParameterInfo`, and
`MBeanOperationInfo` objects contained in the `MBeanInfo`
will have a field `openType` whose value is the [`OpenType`](openmbean/OpenType.md "class in javax.management.openmbean")
specified by the mapping rules above. So even when `getType()`
is "`int`", `getDescriptor().getField("openType")` will
be [`SimpleType.INTEGER`](openmbean/SimpleType.md#INTEGER).
...
is described in the section [Type Names](#type-names)
below.
...
returned by [`Class.getName()`](../../../java.base/java/lang/Class.md#getName()). Otherwise it is the value of
*genericstring(T)*, defined as follows:

* If *T* is a non-generic non-array type,
  *genericstring(T)* is the value returned by [`Class.getName()`](../../../java.base/java/lang/Class.md#getName()), for example `"int"` or `"java.lang.String"`.* If *T* is an array *E[]*,
    *genericstring(T)* is *genericstring(E)* followed
    by `"[]"`. For example, *genericstring(`int[]`)*
    is `"int[]"`, and *genericstring(`List<String>[][]`)* is `"java.util.List<java.lang.String>[][]"`.* Otherwise, *T* is a parameterized type such as `List<String>` and *genericstring(T)* consists of the
...
Open types is signaled with an [`OpenDataException`](openmbean/OpenDataException.md "class in javax.management.openmbean"). This
can happen when an MXBean interface is being analyzed, for
example if it references a type like [`java.util.Random`](../../../java.base/java/util/Random.md "class in java.util") that has no getters. Or it can happen when an
...
Open types is signaled with an [`InvalidObjectException`](../../../java.base/java/io/InvalidObjectException.md "class in java.io").
...
exception such as [`RuntimeMBeanException`](RuntimeMBeanException.md "class in javax.management") or [`UndeclaredThrowableException`](../../../java.base/java/lang/reflect/UndeclaredThrowableException.md "class in java.lang.reflect"). For every thrown exception,
the condition *C* will be true: "*e* is `OpenDataException` or `InvalidObjectException` (as
appropriate), or *C* is true of *e*.[`getCause()`](../../../java.base/java/lang/Throwable.md#getCause())".
