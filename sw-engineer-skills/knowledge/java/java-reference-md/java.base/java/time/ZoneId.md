...
permits [ZoneOffset](ZoneOffset.md "class in java.time") (not exhaustive)

A time-zone ID, such as `Europe/Paris`.

A `ZoneId` is used to identify the rules used to convert between
an [`Instant`](Instant.md "class in java.time") and a [`LocalDateTime`](LocalDateTime.md "class in java.time").
There are two distinct types of ID:

* Fixed offsets - a fully resolved offset from UTC/Greenwich, that uses
  the same offset for all local date-times* Geographical regions - an area where a specific set of rules for finding
    the offset from UTC/Greenwich apply

Most fixed offsets are represented by [`ZoneOffset`](ZoneOffset.md "class in java.time").
Calling [`normalized()`](#normalized()) on any `ZoneId` will ensure that a
fixed offset ID will be represented as a `ZoneOffset`.

The actual rules, describing when and how the offset changes, are defined by [`ZoneRules`](zone/ZoneRules.md "class in java.time.zone").
...
Region-based IDs are defined by configuration, see [`ZoneRulesProvider`](zone/ZoneRulesProvider.md "class in java.time.zone").
...
The TZDB group defines IDs such as 'Europe/London' or 'America/New\_York'.
...
This approach is designed to allow a [`ZonedDateTime`](ZonedDateTime.md "class in java.time") to be loaded and
...
See Also:
:   * [Serialized Form](../../../serialized-form.md#java.time.ZoneId)
...
* ## Field Details
  ...
  + ### SHORT\_IDS
    ...
    public static final [Map](../util/Map.md "interface in java.util")<[String](../lang/String.md "class in java.lang"),[String](../lang/String.md "class in java.lang")> SHORT\_IDS

    A map of zone overrides to enable the short time-zone names to be used.

    Use of short zone IDs has been deprecated in `java.util.TimeZone`.
    This map allows the IDs to continue to be used via the
    [`of(String, Map)`](#of(java.lang.String,java.util.Map)) factory method.
    ...
    - VST - Asia/Ho\_Chi\_MinhThe map is unmodifiable.
* ## Method Details

  + ### systemDefault

    public static [ZoneId](ZoneId.md "class in java.time") systemDefault()

    Gets the system default time-zone.

    This queries [`TimeZone.getDefault()`](../util/TimeZone.md#getDefault()) to find the default time-zone
  ...
  + ### getAvailableZoneIds
    ...
    The ID can be passed to [`of(String)`](#of(java.lang.String)) to create a `ZoneId`.
  ...
  + ### of
    ...
    public static [ZoneId](ZoneId.md "class in java.time") of([String](../lang/String.md "class in java.lang") zoneId,
    [Map](../util/Map.md "interface in java.util")<[String](../lang/String.md "class in java.lang"),[String](../lang/String.md "class in java.lang")> aliasMap)
  ...
  + ### of
    ...
    The result will always be a valid ID for which [`ZoneRules`](zone/ZoneRules.md "class in java.time.zone") can be obtained.

    Parsing matches the zone ID step by step as follows.
    - If the zone ID equals 'Z', the result is `ZoneOffset.UTC`.- If the zone ID consists of a single letter, the zone ID is invalid
        and `DateTimeException` is thrown.- If the zone ID starts with '+' or '-', the ID is parsed as a
          `ZoneOffset` using [`ZoneOffset.of(String)`](ZoneOffset.md#of(java.lang.String)).- If the zone ID equals 'GMT', 'UTC' or 'UT' then the result is a `ZoneId`
            with the same ID and rules equivalent to `ZoneOffset.UTC`.- If the zone ID starts with 'UTC+', 'UTC-', 'GMT+', 'GMT-', 'UT+' or 'UT-'
              then the ID is a prefixed offset-based ID. The ID is split in two, with
              a two or three letter prefix and a suffix starting with the sign.
              The suffix is parsed as a [`ZoneOffset`](ZoneOffset.md#of(java.lang.String)).
              The result will be a `ZoneId` with the specified UTC/GMT/UT prefix
              and the normalized offset ID as per [`ZoneOffset.getId()`](ZoneOffset.md#getId()).
              The rules of the returned `ZoneId` will be equivalent to the
              parsed `ZoneOffset`.- All other IDs are parsed as region-based zone IDs. Region IDs must
                ...
                This has region IDs of the form '{area}/{city}', such as 'Europe/Paris' or 'America/New\_York'.
                This is compatible with most IDs from [`TimeZone`](../util/TimeZone.md "class in java.util").
  ...
  + ### ofOffset
    ...
    public static [ZoneId](ZoneId.md "class in java.time") ofOffset([String](../lang/String.md "class in java.lang") prefix,
  ...
  + ### from
    ...
    public static [ZoneId](ZoneId.md "class in java.time") from([TemporalAccessor](temporal/TemporalAccessor.md "interface in java.time.temporal") temporal)
    ...
    zones over offset-based zones using [`TemporalQueries.zone()`](temporal/TemporalQueries.md#zone()).
  ...
  + ### getId
    ...
    public abstract [String](../lang/String.md "class in java.lang") getId()

    Gets the unique time-zone ID.

    This ID uniquely defines this object.
    The format of an offset based ID is defined by [`ZoneOffset.getId()`](ZoneOffset.md#getId()).
  ...
  + ### getDisplayName
    ...
    If no textual mapping is found then the [`full ID`](#getId()) is returned.
  ...
  + ### getRules
    ...
    public abstract [ZoneRules](zone/ZoneRules.md "class in java.time.zone") getRules()
    ...
    The rules are supplied by [`ZoneRulesProvider`](zone/ZoneRulesProvider.md "class in java.time.zone"). An advanced provider may
    support dynamic updates to the rules without restarting the Java Runtime.
    If so, then the result of this method may change over time.
    Each individual call will be still remain thread-safe.

    [`ZoneOffset`](ZoneOffset.md "class in java.time") will always return a set of rules where the offset never changes.
