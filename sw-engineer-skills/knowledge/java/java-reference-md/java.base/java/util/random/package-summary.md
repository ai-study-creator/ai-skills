...
The principal interface is [`RandomGenerator`](RandomGenerator.md "interface in java.util.random"), which provides
...
The principal supporting class is [`RandomGeneratorFactory`](RandomGeneratorFactory.md "class in java.util.random"). This
can be used to generate multiple random number generators for a specific
algorithm. [`RandomGeneratorFactory`](RandomGeneratorFactory.md "class in java.util.random") also provides methods for
selecting random number generator algorithms. RandomGeneratorFactory
registers implementations of [`RandomGenerator`](RandomGenerator.md "interface in java.util.random") interface using the
service provider API.

An important subsidiary interface is
[`RandomGenerator.StreamableGenerator`](RandomGenerator.StreamableGenerator.md "interface in java.util.random"), which provides methods for
creating spliterator-based streams of [`RandomGenerator`](RandomGenerator.md "interface in java.util.random") objects,
allowing for parallel processing of these objects using multiple threads.
Unlike [`Random`](../Random.md "class in java.util"), most implementations of
[`RandomGenerator`](RandomGenerator.md "interface in java.util.random") are *not* thread-safe. The intent is that
...
generators [`SplittableGenerator`](RandomGenerator.SplittableGenerator.md "interface in java.util.random"),
[`JumpableGenerator`](RandomGenerator.JumpableGenerator.md "interface in java.util.random"),
[`LeapableGenerator`](RandomGenerator.LeapableGenerator.md "interface in java.util.random"), and
[`ArbitrarilyJumpableGenerator`](RandomGenerator.ArbitrarilyJumpableGenerator.md "interface in java.util.random")
...
[`java.util.random`](package-summary.md) has been imported:
> `import java.util.random.*;`

Then one can choose a specific implementation by giving the name of a generator
algorithm to the static method [`RandomGenerator.of(java.lang.String)`](RandomGenerator.md#of(java.lang.String)), in which case the
...
[`nextLong()`](RandomGenerator.md#nextLong()),
[`nextInt()`](RandomGenerator.md#nextInt()),
[`nextFloat()`](RandomGenerator.md#nextFloat()),
[`nextDouble()`](RandomGenerator.md#nextDouble()) and
[`nextBoolean()`](RandomGenerator.md#nextBoolean()) to generate individual
randomly chosen values. One can also use the methods
[`ints()`](RandomGenerator.md#ints()), [`longs()`](RandomGenerator.md#longs())
and [`doubles()`](RandomGenerator.md#doubles()) to create streams of randomly
chosen values. The methods
[`nextGaussian()`](RandomGenerator.md#nextGaussian()) and
[`nextExponential()`](RandomGenerator.md#nextExponential()) draw floating-point
values from nonuniform distributions.

For a multi-threaded application, one can repeat the preceding steps
to create additional [RandomGenerators](RandomGenerator.md "interface in java.util.random"), but
...
interface [`RandomGenerator.StreamableGenerator`](RandomGenerator.StreamableGenerator.md "interface in java.util.random"), then the method
[`rngs()`](RandomGenerator.StreamableGenerator.md#rngs()) can be used to
create a stream of generators. If this is a parallel stream, then it is
easy to get parallel execution by using the
[`map()`](../stream/Stream.md#map(java.util.function.Function)) method on the stream.
...
[`RandomGenerator.SplittableGenerator`](RandomGenerator.SplittableGenerator.md "interface in java.util.random"), which is then considered to
"belong" to the initial thread for its exclusive use; then whenever any
thread needs to fork a new thread, it first uses the
[`split()`](RandomGenerator.SplittableGenerator.md#split()) method of its own
...
secure, then it should continue to use an instance of the class [`SecureRandom`](../../security/SecureRandom.md "class in java.security").
...
provides multiple implementations of interface [`RandomGenerator`](RandomGenerator.md "interface in java.util.random") that
...
In addition, as another life-cycle phase, an algorithm may be [deprecated](RandomGeneratorFactory.md#isDeprecated()). A deprecated algorithm is
...
[`ThreadLocalRandom.current()`](../concurrent/ThreadLocalRandom.md#current()).
...
supported by the interface [`RandomGenerator.JumpableGenerator`](RandomGenerator.JumpableGenerator.md "interface in java.util.random").
...
interface [`RandomGenerator.LeapableGenerator`](RandomGenerator.LeapableGenerator.md "interface in java.util.random"). In this package,
implementations of this interface include "Xoroshiro128PlusPlus" and
"Xoshiro256PlusPlus". There is also an interface
[`RandomGenerator.ArbitrarilyJumpableGenerator`](RandomGenerator.ArbitrarilyJumpableGenerator.md "interface in java.util.random") for algorithms that allow
...
supported by the interface [`RandomGenerator.SplittableGenerator`](RandomGenerator.SplittableGenerator.md "interface in java.util.random").
...
[`SplittableRandom`](../SplittableRandom.md "class in java.util") also implements this interface.
...
*Software: Practice and Experience* (2021), doi:10.1002/spe.3030),
...
SplitMix algorithm (2014) used by `SplittableRandom`
(see Steele and Vigna, "LXM: Better Splittable Pseudorandom Number
Generators (and Almost as Fast)", *Proc. 2021 ACM OOPSLA Conference*).
Each class with a name of the form
`L`*p*`X`*q*`SomethingRandom`
...
where `s`, `m`, and `a` are all binary integers of the same
size, each having *p* bits; `s` is the mutable state, the
multiplier `m` is fixed (the same for all instances of a class) and the
addend `a` is a parameter (a final field of the instance). The
parameter `a` is required to be odd (this allows the LCG to have the
maximal period, namely 2*p*); therefore there are
2*p*−1 distinct choices of parameter. (When the size of
`s` is 128 bits, then we use the name "`sh`" below to refer to
the high half of `s`, that is, the high-order 64 bits of `s`.)

The XBG subgenerator can in principle be any one of a wide variety
of XBG algorithms; in this package it is always either
`xoroshiro128`, `xoshiro256`, or `xoroshiro1024`, in each
...
some fixed number of `int` or `long` fields, generally named
`x0`, `x1`, and so on, which can take on any values provided that
...
Generally speaking, among the "`L`*p*`X`*q*"
generators, the memory required for an instance is 2*p*+*q* bits.
...
approximately true when *p* is 128). A class with "`Mix`" in its
name uses a fairly strong mixing function with excellent avalanche
characteristics; a class with "`StarStar`" in its name uses a weaker
but faster mixing function.

The specific LXM algorithms used in this package are all chosen so that
the 64-bit values produced by the [`nextLong()`](RandomGenerator.md#nextLong())
...
264 possible `long` values will be produced
2128−1 times). The values produced by the
[`nextInt()`](RandomGenerator.md#nextInt()),
[`nextFloat()`](RandomGenerator.md#nextFloat()), and
[`nextDouble()`](RandomGenerator.md#nextDouble()) methods are likewise exactly
...
[`nextLong()`](RandomGenerator.md#nextLong()) method are exactly
...
| Implementation | Period | State size | Parameter size | [`nextLong()`](RandomGenerator.md#nextLong()) values are |
| --- | --- | --- | --- | --- |
...
| "L128X1024MixRandom" | 2128(21024−1) | 1152 bits | 128 bits | exactly equidistributed |

For the algorithms listed above whose names begin with `L32`, the
32-bit values produced by the [`nextInt()`](RandomGenerator.md#nextInt())
method are exactly equidistributed, but the 64-bit values produced by the
[`nextLong()`](RandomGenerator.md#nextLong()) method are not exactly
equidistributed.

For the algorithms listed above whose names begin with `L64` or
`L128`, the 64-bit values produced by the
[`nextLong()`](RandomGenerator.md#nextLong()) method are *exactly
equidistributed*: every instance, over the course of its cycle, will
produce each of the 264 possible `long` values exactly the
...
`L64`, the 64-bit values produced by the
[`nextLong()`](RandomGenerator.md#nextLong()) method are
...
[`nextLong()`](RandomGenerator.md#nextLong()) (assuming no other methods are
...
[`nextInt()`](RandomGenerator.md#nextInt()),
[`nextFloat()`](RandomGenerator.md#nextFloat()), and
[`nextDouble()`](RandomGenerator.md#nextDouble()) methods are likewise
...
cases is 65 bits wide, so the constant `0x1d605bbb58c8abbfdL` shown in
the table cannot actually be used in code; instead, only the 64 low-order
bits `0xd605bbb58c8abbfdL` are represented in the source code, and the
missing 1-bit is handled through special coding of the multiply-add algorithm
used in the LCG.)

LXM Multipliers

| Implementation | LCG multiplier `m` | XBG algorithm | XBG parameters | Mixing function |
| --- | --- | --- | --- | --- |
| "L32X64MixRandom" | `0xadb4a92d` | `xoroshiro64`, version 1.0 | `(26, 9, 13)` | mixLea32`(s+x0)` |
| "L64X128StarStarRandom" | `0xd1342543de82ef95L` | `xoroshiro128`, version 1.0 | `(24, 16, 37)` | `Long.rotateLeft((s+x0)* 5, 7) * 9` |
| "L64X128MixRandom" | `0xd1342543de82ef95L` | `xoroshiro128`, version 1.0 | `(24, 16, 37)` | mixLea64`(s+x0)` |
| "L64X256MixRandom" | `0xd1342543de82ef95L` | `xoshiro256`, version 1.0 | `(17, 45)` | mixLea64`(s+x0)` |
| "L64X1024MixRandom" | `0xd1342543de82ef95L` | `xoroshiro1024`, version 1.0 | `(25, 27, 36)` | mixLea64`(s+x0)` |
| "L128X128MixRandom" | `0x1d605bbb58c8abbfdL` | `xoroshiro128`, version 1.0 | `(24, 16, 37)` | mixLea64`(sh+x0)` |
| "L128X256MixRandom" | `0x1d605bbb58c8abbfdL` | `xoshiro256`, version 1.0 | `(17, 45)` | mixLea64`(sh+x0)` |
| "L128X1024MixRandom" | `0x1d605bbb58c8abbfdL` | `xoroshiro1024`, version 1.0 | `(25, 27, 36)` | mixLea64`(sh+x0)` |
...
* All Classes and InterfacesInterfacesClasses
  ...
  [RandomGenerator](RandomGenerator.md "interface in java.util.random")
  ...
  [RandomGenerator.ArbitrarilyJumpableGenerator](RandomGenerator.ArbitrarilyJumpableGenerator.md "interface in java.util.random")
  ...
  [RandomGenerator.JumpableGenerator](RandomGenerator.JumpableGenerator.md "interface in java.util.random")

  This interface is designed to provide a common protocol for objects that
  generate pseudorandom values and can easily *jump* forward, by a
  moderate amount (ex.

  [RandomGenerator.LeapableGenerator](RandomGenerator.LeapableGenerator.md "interface in java.util.random")

  This interface is designed to provide a common protocol for objects that
  generate sequences of pseudorandom values and can easily not only jump
  but also *leap* forward, by a large amount (ex.

  [RandomGenerator.SplittableGenerator](RandomGenerator.SplittableGenerator.md "interface in java.util.random")
  ...
  [RandomGenerator.StreamableGenerator](RandomGenerator.StreamableGenerator.md "interface in java.util.random")
  ...
  [RandomGeneratorFactory](RandomGeneratorFactory.md "class in java.util.random")<T extends [RandomGenerator](RandomGenerator.md "interface in java.util.random")>
