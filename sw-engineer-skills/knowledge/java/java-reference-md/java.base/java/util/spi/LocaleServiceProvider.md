...
[`DateFormatProvider`](../../text/spi/DateFormatProvider.md "class in java.text.spi") class should
...
Otherwise, they call the [`isSupportedLocale`](#isSupportedLocale(java.util.Locale))
methods of installed providers for the appropriate interface to find one that
supports the requested locale. If such a provider is found, its other
methods are called to obtain the requested object or name. When checking
whether a locale is supported, the [locale's extensions](../Locale.md#def_extensions) are ignored by default. (If locale's extensions should
...
[`getCandidateLocales`](../ResourceBundle.Control.md#getCandidateLocales(java.lang.String,java.util.Locale))
...
[`System.setProperty(String, String)`](../../lang/System.md#setProperty(java.lang.String,java.lang.String)) is discouraged and it may not affect
the order.
JDK Reference Implementation provides the following four
locale providers:

* "CLDR": A provider based on Unicode Consortium's
  [CLDR Project](http://cldr.unicode.org/).* "COMPAT": represents the locale sensitive services that is compatible
    with the prior JDK releases up to JDK 8 (same as JDK 8's "JRE"). This
    provider is deprecated and will be removed in the future release of JDK.* "SPI": represents the locale sensitive services implementing the subclasses of
      this `LocaleServiceProvider` class.* "HOST": A provider that reflects the user's custom settings in the
        underlying operating system. This provider may not be available, depending
        on the JDK Reference Implementation.* "JRE": represents a synonym to "COMPAT". This name
...
* ## Constructor Details
  ...
  + ### LocaleServiceProvider
    ...
    :   `SecurityException` - If a security manager has been installed and it denies
        [`RuntimePermission("localeServiceProvider")`](../../lang/RuntimePermission.md "class in java.lang")
* ## Method Details

  + ### getAvailableLocales

    public abstract [Locale](../Locale.md "class in java.util")[] getAvailableLocales()
  ...
  + ### isSupportedLocale
    ...
    public boolean isSupportedLocale([Locale](../Locale.md "class in java.util") locale)

    Returns `true` if the given `locale` is supported by
    this locale service provider. The given `locale` may contain
    [extensions](../Locale.md#def_extensions) that should be
    ...
    [`getAvailableLocales()`](#getAvailableLocales()) with ignoring any extensions in both the
    ...
    See Also:
    :   - [`Locale.hasExtensions()`](../Locale.md#hasExtensions())
        - [`Locale.stripExtensions()`](../Locale.md#stripExtensions())