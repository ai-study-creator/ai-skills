...
example of a [reduction](package-summary.md#Reduction)
operation.)

The key abstraction introduced in this package is *stream*. The
classes [`Stream`](Stream.md "interface in java.util.stream"), [`IntStream`](IntStream.md "interface in java.util.stream"),
[`LongStream`](LongStream.md "interface in java.util.stream"), and [`DoubleStream`](DoubleStream.md "interface in java.util.stream")
...
* Functional in nature. An operation on a stream produces a result,
  but does not modify its source. For example, filtering a `Stream`
...
* Laziness-seeking. Many stream operations, such as filtering, mapping,
  ...
  for optimization. For example, "find the first `String` with
...
* Consumable. The elements of a stream are only visited once during
  the life of a stream. Like an [`Iterator`](../Iterator.md "interface in java.util"), a new stream
  must be generated to revisit the same elements of the source.

Streams can be obtained in a number of ways. Some examples include:

* From a [`Collection`](../Collection.md "interface in java.util") via the `stream()` and
  `parallelStream()` methods;
* From an array via [`Arrays.stream(Object[])`](../Arrays.md#stream(T%5B%5D));
* From static factory methods on the stream classes, such as
  [`Stream.of(Object[])`](Stream.md#of(T...)),
  [`IntStream.range(int, int)`](IntStream.md#range(int,int))
  or [`Stream.iterate(Object, UnaryOperator)`](Stream.md#iterate(T,java.util.function.UnaryOperator));
* The lines of a file can be obtained from [`BufferedReader.lines()`](../../io/BufferedReader.md#lines());
* Streams of file paths can be obtained from methods in [`Files`](../../nio/file/Files.md "class in java.nio.file");
* Streams of random numbers can be obtained from [`Random.ints()`](../Random.md#ints());
* Numerous other stream-bearing methods in the JDK, including
  [`BitSet.stream()`](../BitSet.md#stream()),
  [`Pattern.splitAsStream(java.lang.CharSequence)`](../regex/Pattern.md#splitAsStream(java.lang.CharSequence)),
  and [`JarFile.stream()`](../jar/JarFile.md#stream()).

Additional stream sources can be provided by third-party libraries using
[these techniques](package-summary.md#StreamSources).
...
[`Collection.stream()`](../Collection.md#stream()) and [`Collection.parallelStream()`](../Collection.md#parallelStream()),
which produce sequential and parallel streams respectively; other
stream-bearing methods such as [`IntStream.range(int, int)`](IntStream.md#range(int,int))
produce sequential streams but these streams can be efficiently parallelized by
invoking their [`BaseStream.parallel()`](BaseStream.md#parallel()) method.
...
instead of "`stream()`". The stream pipeline is executed sequentially or
...
[`BaseStream.isParallel()`](BaseStream.md#isParallel()) method, and the
stream's mode can be modified with the
[`BaseStream.sequential()`](BaseStream.md#sequential()) and
[`BaseStream.parallel()`](BaseStream.md#parallel()) operations.
...
of a [functional interface](../function/package-summary.md) such
...
[Low-level stream
construction](package-summary.md#StreamSources) for requirements for building well-behaved streams.
...
* the [*visibility*](../concurrent/package-summary.md#MemoryVisibility) of those side-effects to other threads;
...
constrained to produce a *result* that is consistent with the
...
terminal operations [`forEach`](Stream.md#forEach(java.util.function.Consumer)) and
[`forEachOrdered`](Stream.md#forEachOrdered(java.util.function.Consumer)), side-effects
...
see the API note documented on the [`count`](Stream.md#count())
operation.)

Many computations where one might be tempted to use side-effects can be more
safely and efficiently expressed without side-effects, such as using
[reduction](package-summary.md#Reduction) instead of mutable
...
ordered stream unordered, such as [`BaseStream.unordered()`](BaseStream.md#unordered()).
...
the stream with [`unordered()`](BaseStream.md#unordered()) may
...
[`reduce()`](Stream.md#reduce(java.util.function.BinaryOperator))
and [`collect()`](Stream.md#collect(java.util.stream.Collector)),
as well as multiple specialized reduction forms such as
[`sum()`](IntStream.md#sum()), [`max()`](IntStream.md#max()),
or [`count()`](IntStream.md#count()).
...
are [associative](package-summary.md#Associativity) and
...
`combiner` function must be [associative](package-summary.md#Associativity) and
...
the results into a [`StringBuilder`](../../lang/StringBuilder.md "class in java.lang"), which is a mutable
...
[`collect()`](Stream.md#collect(java.util.stream.Collector)),
...
Here, our supplier is just the [`ArrayList constructor`](../ArrayList.md#%3Cinit%3E()), the accumulator adds the stringified element to an
`ArrayList`, and the combiner simply uses [`addAll`](../ArrayList.md#addAll(java.util.Collection))
...
[`Collector`](Collector.md "interface in java.util.stream") to capture all three aspects. The
...
composability. The class [`Collectors`](Collectors.md "class in java.util.stream") contains a
...
[`groupingBy`](Collectors.md#groupingBy(java.util.function.Function,java.util.stream.Collector)):
...
Here, equivalence generally means according to [`Object.equals(Object)`](../../lang/Object.md#equals(java.lang.Object)).
...
[`ConcurrentHashMap`](../concurrent/ConcurrentHashMap.md "class in java.util.concurrent"). In that case, the parallel
...
A [`Collector`](Collector.md "interface in java.util.stream") that supports concurrent reduction is
marked with the [`Collector.Characteristics.CONCURRENT`](Collector.Characteristics.md#CONCURRENT)
...
stream being processed. The [`Stream.collect(Collector)`](Stream.md#collect(java.util.stream.Collector))
implementation will only perform a concurrent reduction if

* The stream is parallel;
* The collector has the
  [`Collector.Characteristics.CONCURRENT`](Collector.Characteristics.md#CONCURRENT) characteristic,
  and;
* Either the stream is unordered, or the collector has the
  [`Collector.Characteristics.UNORDERED`](Collector.Characteristics.md#UNORDERED) characteristic.

You can ensure the stream is unordered by using the
[`BaseStream.unordered()`](BaseStream.md#unordered()) method. For example:
...
(where [`Collectors.groupingByConcurrent(java.util.function.Function<? super T, ? extends K>)`](Collectors.md#groupingByConcurrent(java.util.function.Function)) is the
...
[`Collection.stream()`](../Collection.md#stream()) or [`Arrays.stream(Object[])`](../Arrays.md#stream(T%5B%5D))
to obtain a stream. How are those stream-bearing methods implemented?

The class [`StreamSupport`](StreamSupport.md "class in java.util.stream") has a number of
low-level methods for creating a stream, all using some form of a
[`Spliterator`](../Spliterator.md "interface in java.util"). A spliterator is the parallel analogue of an
[`Iterator`](../Iterator.md "interface in java.util"); it describes a (possibly infinite) collection of
...
[`Spliterators.spliteratorUnknownSize(java.util.Iterator, int)`](../Spliterators.md#spliteratorUnknownSize(java.util.Iterator,int)).
...
[`characteristics`](../Spliterator.md#characteristics()) of the
...
[*late-binding*](../Spliterator.md#binding). If a source
...
[`stream()`](StreamSupport.md#stream(java.util.function.Supplier,int,boolean)).
...
[Non-Interference](package-summary.md#NonInterference)
for more details.
...
* All Classes and InterfacesInterfacesClassesEnum Classes
  ...
  [BaseStream](BaseStream.md "interface in java.util.stream")<T,S extends [BaseStream](BaseStream.md "interface in java.util.stream")<T,S>>

  Base interface for streams, which are sequences of elements supporting
  sequential and parallel aggregate operations.

  [Collector](Collector.md "interface in java.util.stream")<T,A,R>
  ...
  [Collector.Characteristics](Collector.Characteristics.md "enum class in java.util.stream")
  ...
  [DoubleStream.Builder](DoubleStream.Builder.md "interface in java.util.stream")

  A mutable builder for a `DoubleStream`.

  [DoubleStream.DoubleMapMultiConsumer](DoubleStream.DoubleMapMultiConsumer.md "interface in java.util.stream")
  ...
  [IntStream.Builder](IntStream.Builder.md "interface in java.util.stream")

  A mutable builder for an `IntStream`.

  [IntStream.IntMapMultiConsumer](IntStream.IntMapMultiConsumer.md "interface in java.util.stream")
  ...
  [LongStream.Builder](LongStream.Builder.md "interface in java.util.stream")

  A mutable builder for a `LongStream`.

  [LongStream.LongMapMultiConsumer](LongStream.LongMapMultiConsumer.md "interface in java.util.stream")

  Represents an operation that accepts a `long`-valued argument
  and a LongConsumer, and returns no result.

  [Stream](Stream.md "interface in java.util.stream")<T>

  A sequence of elements supporting sequential and parallel aggregate
  operations.

  [Stream.Builder](Stream.Builder.md "interface in java.util.stream")<T>

  A mutable builder for a `Stream`.

  [StreamSupport](StreamSupport.md "class in java.util.stream")
