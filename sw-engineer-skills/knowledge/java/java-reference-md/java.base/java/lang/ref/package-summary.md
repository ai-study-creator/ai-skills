...
[`Cleaner`](Cleaner.md "class in java.lang.ref").

Each reference-object type is implemented by a subclass of the
abstract base [`Reference`](Reference.md "class in java.lang.ref") class.
...
are otherwise immutable, so no `set` operation is
...
the [`ReferenceQueue`](ReferenceQueue.md "class in java.lang.ref") class.
...
[`WeakHashMap`](../../util/WeakHashMap.md "class in java.util") class works. Because
the [`ReferenceQueue.poll`](ReferenceQueue.md#poll()) method simply checks an internal data
...
* An object is *strongly reachable* if it can be reached
  ...
  created it.* An object is *softly reachable* if it is not strongly
    reachable but can be reached by traversing a soft reference.* An object is *weakly reachable* if it is neither
      ...
      object are cleared, the object becomes eligible for finalization.* An object is *phantom reachable* if it is neither
        strongly, softly, nor weakly reachable, it has been finalized, and
        some phantom reference refers to it.* Finally, an object is *unreachable*, and therefore
...
* All Classes and InterfacesInterfacesClasses
  ...
  [Cleaner.Cleanable](Cleaner.Cleanable.md "interface in java.lang.ref")

  `Cleanable` represents an object and a
  cleaning action registered in a `Cleaner`.

  [PhantomReference](PhantomReference.md "class in java.lang.ref")<T>

  Phantom reference objects, which are enqueued after the collector
  determines that their referents may otherwise be reclaimed.

  [Reference](Reference.md "class in java.lang.ref")<T>

  Abstract base class for reference objects.

  [ReferenceQueue](ReferenceQueue.md "class in java.lang.ref")<T>

  Reference queues, to which registered reference objects are appended by the
  garbage collector after the appropriate reachability changes are detected.

  [SoftReference](SoftReference.md "class in java.lang.ref")<T>

  Soft reference objects, which are cleared at the discretion of the garbage
  collector in response to memory demand.

  [WeakReference](WeakReference.md "class in java.lang.ref")<T>
