...
object, which implements the interface [`InvocationHandler`](InvocationHandler.md "interface in java.lang.reflect").
A method invocation on a proxy instance through one of its proxy
interfaces will be dispatched to the [`invoke`](InvocationHandler.md#invoke(java.lang.Object,java.lang.reflect.Method,java.lang.Object%5B%5D)) method of the instance's invocation handler, passing the proxy
...
* The unqualified name of a proxy class is unspecified. The space
  ...
  should be, however, reserved for proxy classes.* The package and module in which a proxy class is defined is specified
    [below](#membership).* A proxy class is *final and non-abstract*.* A proxy class extends `java.lang.reflect.Proxy`.* A proxy class implements exactly the interfaces specified at its
          creation, in the same order. Invoking [`getInterfaces`](../Class.md#getInterfaces())
          on its `Class` object will return an array containing the same
          list of interfaces (in the order specified at its creation), invoking
          [`getMethods`](../Class.md#getMethods()) on its `Class` object will return
          an array of `Method` objects that include all of the
          methods in those interfaces, and invoking `getMethod` will
          find methods in the proxy interfaces as would be expected.* The [`ProtectionDomain`](../../security/ProtectionDomain.md "class in java.security") of a proxy class
            is the same as that of system classes loaded by the bootstrap class
            loader, such as `java.lang.Object`, because the code for a
            proxy class is generated by trusted system code. This protection
            domain will typically be granted `java.security.AllPermission`.* The [`Proxy.isProxyClass`](#isProxyClass(java.lang.Class)) method can be used
...
* Given a proxy instance `proxy` and one of the
  ...
  * Each proxy instance has an associated invocation handler, the one
    ...
    [`Proxy.getInvocationHandler`](#getInvocationHandler(java.lang.Object)) method
    will return the invocation handler associated with the proxy instance
    passed as its argument.* An interface method invocation on a proxy instance will be
      encoded and dispatched to the invocation handler's [`invoke`](InvocationHandler.md#invoke(java.lang.Object,java.lang.reflect.Method,java.lang.Object%5B%5D)) method as described in the
      documentation for that method.* A proxy interface may define a default method or inherit
        ...
        by calling [`InvocationHandler::invokeDefault`](InvocationHandler.md#invokeDefault(java.lang.Object,java.lang.reflect.Method,java.lang.Object...)).* An invocation of the `hashCode`,
...
[`getProxyClass(ClassLoader, Class[])`](#getProxyClass(java.lang.ClassLoader,java.lang.Class...)) or
[`newProxyInstance(ClassLoader, Class[], InvocationHandler)`](#newProxyInstance(java.lang.ClassLoader,java.lang.Class%5B%5D,java.lang.reflect.InvocationHandler))
methods is specified as follows:

1. If all the proxy interfaces are in *exported* or *open*
   packages:
   1. if all the proxy interfaces are *public*, then the proxy class is
      *public* in an unconditionally exported but non-open package.
      The name of the package and the module are unspecified.
   2. if at least one of all the proxy interfaces is *non-public*, then
      ...
      [not possible](#restrictions).
2. If at least one proxy interface is in a package that is
...
in an open package with [`setAccessible`](AccessibleObject.md#setAccessible(java.lang.reflect.AccessibleObject%5B%5D,boolean)),
...
Calling [`Constructor.newInstance(Object...)`](Constructor.md#newInstance(java.lang.Object...)) on a proxy class in
...
the [module](../Module.md "class in java.lang") of `T` is updated to export the
...
:   * [`InvocationHandler`](InvocationHandler.md "interface in java.lang.reflect")
    * [Serialized Form](../../../../serialized-form.md#java.lang.reflect.Proxy)
...
* ## Field Details
  ...
  + ### h
    ...
    protected [InvocationHandler](InvocationHandler.md "interface in java.lang.reflect") h
...
* ## Constructor Details
  ...
  + ### Proxy
    ...
    protected Proxy([InvocationHandler](InvocationHandler.md "interface in java.lang.reflect") h)
    ...
    Throws:
    :   `NullPointerException` - if the given invocation handler, `h`,
        is `null`.
* ## Method Details
  ...
  + ### getProxyClass
    ...
    public static [Class](../Class.md "class in java.lang")<?> getProxyClass([ClassLoader](../ClassLoader.md "class in java.lang") loader,
    [Class](../Class.md "class in java.lang")<?>... interfaces)
    ...
    Proxy classes generated in a named module are encapsulated
    and not accessible to code outside its module.
    [`Constructor.newInstance`](Constructor.md#newInstance(java.lang.Object...))
    will throw `IllegalAccessException` when it is called on
    an inaccessible proxy class.
    Use [`newProxyInstance(ClassLoader, Class[], InvocationHandler)`](#newProxyInstance(java.lang.ClassLoader,java.lang.Class%5B%5D,java.lang.reflect.InvocationHandler))
    ...
    Throws:
    :   `IllegalArgumentException` - if any of the [restrictions](#restrictions) on the parameters are violated
    :   `SecurityException` - if a security manager, *s*, is present
        and any of the following conditions is met:
        - the given `loader` is `null` and
          the caller's class loader is not `null` and the
          invocation of [`s.checkPermission`](../SecurityManager.md#checkPermission(java.security.Permission)) with
        ...
        - for each proxy interface, `intf`,
          ...
          invocation of [`s.checkPackageAccess()`](../SecurityManager.md#checkPackageAccess(java.lang.String)) denies access to `intf`.
    :   `NullPointerException` - if the `interfaces` array
        argument or any of its elements are `null`

    See Also:
    :   - [Package and Module Membership of Proxy Class](#membership)
  + ### newProxyInstance

    public static [Object](../Object.md "class in java.lang") newProxyInstance([ClassLoader](../ClassLoader.md "class in java.lang") loader,
    [Class](../Class.md "class in java.lang")<?>[] interfaces,
    [InvocationHandler](InvocationHandler.md "interface in java.lang.reflect") h)
    ...
    - All of `Class` objects in the given `interfaces` array
      must represent [non-hidden](../Class.md#isHidden()) and
      [non-sealed](../Class.md#isSealed()) interfaces,
      not classes or primitive types.- No two elements in the `interfaces` array may
        refer to identical `Class` objects.- All of the interface types must be visible by name through the
          specified class loader. In other words, for class loader
          `cl` and every interface `i`, the following
          expression must be true:

          `Class.forName(i.getName(), false, cl) == i`- All of the types referenced by all
            public method signatures of the specified interfaces
            and those inherited by their superinterfaces
            must be visible by name through the specified class loader.- All non-public interfaces must be in the same package
              ...
              regardless of what package it is defined in.- For any set of member methods of the specified interfaces
                that have the same signature:
                * If the return type of any of the methods is a primitive
                  type or void, then all of the methods must have that same
                  return type.* Otherwise, one of the methods must have a return type that
                    is assignable to all of the return types of the rest of the
                    methods.- The resulting proxy class must not exceed any limits imposed
    ...
    :   `SecurityException` - if a security manager, *s*, is present
        ...
        - any of the given proxy interfaces is non-public and the
          caller class is not in the same [runtime package](../Package.md "class in java.lang")
          as the non-public interface and the invocation of
          [`s.checkPermission`](../SecurityManager.md#checkPermission(java.security.Permission)) with
  ...
  + ### isProxyClass
    ...
    public static boolean isProxyClass([Class](../Class.md "class in java.lang")<?> cl)
  ...
  + ### getInvocationHandler
    ...
    public static [InvocationHandler](InvocationHandler.md "interface in java.lang.reflect") getInvocationHandler([Object](../Object.md "class in java.lang") proxy)
    throws [IllegalArgumentException](../IllegalArgumentException.md "class in java.lang")
    ...
    :   `SecurityException` - if a security manager, *s*, is present
        ...
        and invocation of [`s.checkPackageAccess()`](../SecurityManager.md#checkPackageAccess(java.lang.String)) denies access to the invocation
        handler's class.