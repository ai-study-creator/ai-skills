...
| modifyThread | Modification of threads, e.g., via calls to Thread `interrupt, setDaemon, setPriority, setName` and `setUncaughtExceptionHandler` methods | This allows an attacker to modify the behaviour of any thread in the system. |
| modifyThreadGroup | modification of thread groups, e.g., via calls to ThreadGroup `getParent`, `setDaemon`, and `setMaxPriority` methods | This allows an attacker to create thread groups and set their run priority. |
...
| defineClass | Define a class with [`Lookup.defineClass`](invoke/MethodHandles.Lookup.md#defineClass(byte%5B%5D)). | This grants code with a suitably privileged `Lookup` object permission to define classes in the same package as the `Lookup`'s lookup class. |
...
| getStackWalkerWithClassReference | Get a stack walker that can retrieve stack frames with class reference. | This allows retrieval of Class objects from stack walking. This might allow malicious code to access Class objects on the stack outside its own context. |
...
| manageProcess | Native process termination and information about processes [`ProcessHandle`](ProcessHandle.md "interface in java.lang"). | Allows code to identify and terminate processes that it did not create. |
| localeServiceProvider | This `RuntimePermission` is required to be granted to classes which subclass and implement `java.util.spi.LocaleServiceProvider`. The permission is checked during invocation of the abstract base class constructor. This permission ensures trust in classes which implement this security-sensitive provider mechanism. | See [`java.util.spi.LocaleServiceProvider`](../util/spi/LocaleServiceProvider.md) for more information. |
| loggerFinder | This `RuntimePermission` is required to be granted to classes which subclass or call methods on `java.lang.System.LoggerFinder`. The permission is checked during invocation of the abstract base class constructor, as well as on the invocation of its public methods. This permission ensures trust in classes which provide loggers to system classes. | See [`java.lang.System.LoggerFinder`](System.LoggerFinder.md "class in java.lang") for more information. |
| accessSystemModules | Access system modules in the runtime image. | This grants the permission to access resources in the [system modules](module/ModuleFinder.md#ofSystem()) in the runtime image. |
| inetAddressResolverProvider | This `RuntimePermission` is required to be granted to classes which subclass and implement `java.net.spi.InetAddressResolverProvider`. The permission is checked during invocation of the abstract base class constructor. This permission ensures trust in classes which provide resolvers used by [`InetAddress`](../net/InetAddress.md "class in java.net") hostname and address resolution methods. | See [`InetAddressResolverProvider`](../net/spi/InetAddressResolverProvider.md "class in java.net.spi") for more information. |
...
:   * [`BasicPermission`](../security/BasicPermission.md "class in java.security")
    ...
    * [Serialized Form](../../../serialized-form.md#java.lang.RuntimePermission)
...
* ## Constructor Details
  ...
  + ### RuntimePermission
    ...
    public RuntimePermission([String](String.md "class in java.lang") name)
