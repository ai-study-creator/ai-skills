...
[resolution](package-summary.md#resolution) or
[service binding](Configuration.md#service-binding).
...
The [`find`](#find(java.lang.String)) and [`findAll`](#findAll()) methods
...
case of `ModuleFinder` returned by [`ModuleFinder.of`](#of(java.nio.file.Path...)), that
two or more modules with the same name are found in a directory.
When an error is detected then these methods throw [`FindException`](FindException.md "class in java.lang.module") with an appropriate [`cause`](../Throwable.md#getCause()).
...
* ## Method Details
  ...
  + ### find
    ...
    [Optional](../../util/Optional.md "class in java.util")<[ModuleReference](ModuleReference.md "class in java.lang.module")> find([String](../String.md "class in java.lang") name)
  ...
  + ### findAll
    ...
    [Set](../../util/Set.md "interface in java.util")<[ModuleReference](ModuleReference.md "class in java.lang.module")> findAll()
    ...
    [`find`](#find(java.lang.String)) will locate the `ModuleReference` if invoked
    to find that module.
  ...
  + ### ofSystem
    ...
    static [ModuleFinder](ModuleFinder.md "interface in java.lang.module") ofSystem()
    ...
    If there is a security manager set then its [`checkPermission`](../SecurityManager.md#checkPermission(java.security.Permission)) method is
    invoked to check that the caller has been granted
    [`RuntimePermission("accessSystemModules")`](../RuntimePermission.md "class in java.lang")
  ...
  + ### of
    ...
    static [ModuleFinder](ModuleFinder.md "interface in java.lang.module") of([Path](../../nio/file/Path.md "interface in java.nio.file")... entries)

    Returns a module finder that locates modules on the file system by
    searching a sequence of directories and/or packaged modules.
    Each element in the given array is one of:
    1. A path to a directory of modules.
    2. A path to the *top-level* directory of an
       *exploded module*.
    3. A path to a *packaged module*.The module finder locates modules by searching each directory, exploded
    ...
    in a [multi-release](../../util/jar/JarFile.md#isMultiRelease())
    ...
    - If the JAR file has the attribute "`Automatic-Module-Name`"
      in its main manifest then its value is the [module name](ModuleDescriptor.md#name()). The module name is otherwise
      derived from the name of the JAR file.
    - The [`version`](ModuleDescriptor.md#version()), and the
      ...
      * If the name matches the regular expression `"-(\\d+(\\.|$))"` then the module name will be derived from the
        ...
        subsequence after the hyphen is parsed as a [`Version`](ModuleDescriptor.Version.md "class in java.lang.module") and ignored if it cannot be
        parsed as a `Version`.
    ...
    - The contents of entries starting with `META-INF/services/` are assumed to be service configuration files
      (see [`ServiceLoader`](../../util/ServiceLoader.md "class in java.util")). If the name of a file
    ...
    - If the JAR file has a `Main-Class` attribute in its
      ...
      module [main class](ModuleDescriptor.md#mainClass()).

    If a `ModuleDescriptor` cannot be created (by means of the
    [`ModuleDescriptor.Builder`](ModuleDescriptor.Builder.md "class in java.lang.module") API) for an
    ...
    in the module. Whether [hidden files](../../nio/file/Files.md#isHidden(java.nio.file.Path)) are ignored or not is implementation specific and therefore
    ...
    Parameters:
    :   `entries` - A possibly-empty array of paths to directories of modules
  ...
  + ### compose
    ...
    static [ModuleFinder](ModuleFinder.md "interface in java.lang.module") compose([ModuleFinder](ModuleFinder.md "interface in java.lang.module")... finders)

    Returns a module finder that is composed from a sequence of zero or more
    module finders. The [`find`](#find(java.lang.String)) method of the resulting
    ...
    found or all module finders have been searched. The [`findAll`](#findAll()) method of the resulting module finder will return a set of
