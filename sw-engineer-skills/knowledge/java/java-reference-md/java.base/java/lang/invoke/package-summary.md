...
* The classes [`MethodHandle`](MethodHandle.md "class in java.lang.invoke") and
  [`VarHandle`](VarHandle.md "class in java.lang.invoke") contain
  [signature polymorphic methods](MethodHandle.md#sigpoly)
...
* The `invokedynamic` instruction makes use of bootstrap `MethodHandle`
  constants to dynamically resolve `CallSite` objects for custom method invocation
  behavior.
* The `ldc` instruction makes use of bootstrap `MethodHandle` constants
...
An `invokedynamic` instruction is originally in an unlinked state.
In this state, there is no target method for the instruction to invoke.

Before the JVM can execute an `invokedynamic` instruction,
...
and which must produce a [`CallSite`](CallSite.md "class in java.lang.invoke")
...
just like `invokestatic` and the other invoke instructions.

### Dynamically-computed constants

The constant pool may contain constants tagged `CONSTANT_Dynamic`,
...
just like `getstatic` and the other field reference instructions.
(Roughly speaking, a dynamically-computed constant is to a dynamically-computed call site
as a `CONSTANT_Fieldref` is to a `CONSTANT_Methodref`.)
...
* the `Class` or `MethodType` derived from
  type component of the `CONSTANT_NameAndType` descriptor
* static arguments, if any (note that static arguments can themselves be
  dynamically-computed constants)

The bootstrap method is then invoked, as if by
[`MethodHandle.invoke`](MethodHandle.md#invoke(java.lang.Object...)),
...
[`CallSite`](CallSite.md "class in java.lang.invoke").
The type of the call site's target must be exactly equal to the type
derived from the invocation's type descriptor and passed to
the bootstrap method. If these conditions are not met, a `BootstrapMethodError` is thrown.
...
method must be assignable to `MethodHandles.Lookup`. If this condition
is not met, a `BootstrapMethodError` is thrown.
On success the result of the bootstrap method is cached as the resolved
constant value.

If an exception, `E` say, occurs during execution of the bootstrap method, then
resolution fails and terminates abnormally. `E` is rethrown if the type of
`E` is `Error` or a subclass, otherwise a
`BootstrapMethodError` that wraps `E` is thrown.
If this happens, the same error will be thrown for all
subsequent attempts to execute the `invokedynamic` instruction or load the
...
In an application which requires `invokedynamic` instructions with individually
mutable behaviors, their bootstrap methods should produce distinct
[`CallSite`](CallSite.md "class in java.lang.invoke") objects, one for each linkage request.
...
Every `invokedynamic` instruction transitions at most once from unlinked to linked,
...
types `MethodHandles.Lookup`, `String`, `MethodType`, and the types
...
`MethodHandles.Lookup`, `String`, `Class`, and the types of any
static arguments; the return type is the type represented by the `Class`.

Because [`MethodHandle.invoke`](MethodHandle.md#invoke(java.lang.Object...)) allows for
...
must be assignable to `MethodHandles.Lookup`, other than that constraint the same degree
...
could be `Object` instead of `MethodHandles.Lookup`, and the return type
...
and the [`asVarargsCollector`](MethodHandle.md#asVarargsCollector(java.lang.Class)) transformation.)

Given these rules, here are examples of legal bootstrap method declarations for
dynamically-computed call sites, given various numbers `N` of extra arguments.
The first row (marked `*`) will work for any number of extra arguments.
...
`String` and `Integer` (or `int`), respectively.
...
constant's declared type (such as `Object`, which is always compatible).
...
However, arguments of type `boolean`, `byte`, `short`, or `char`
cannot be *directly* supplied by `CONSTANT_Integer`
constant pool entries, since the `asType` conversions do
...
(using the `asType` conversions) to `CallSite`, which
means the bootstrap method return type might be `Object` or
`ConstantCallSite`.
...
`Object`, `Character`, or `char`, but not
`int` or `Integer`.
...
* All Classes and InterfacesInterfacesClassesEnum ClassesException Classes
  ...
  [ConstantBootstraps](ConstantBootstraps.md "class in java.lang.invoke")
  ...
  [LambdaConversionException](LambdaConversionException.md "class in java.lang.invoke")
  ...
  [MethodHandleProxies](MethodHandleProxies.md "class in java.lang.invoke")
  ...
  [MethodHandles.Lookup.ClassOption](MethodHandles.Lookup.ClassOption.md "enum class in java.lang.invoke")
  ...
  A `MutableCallSite` is a [`CallSite`](CallSite.md "class in java.lang.invoke") whose target variable
  behaves like an ordinary field.

  [SerializedLambda](SerializedLambda.md "class in java.lang.invoke")

  Serialized form of a lambda expression.

  [StringConcatException](StringConcatException.md "class in java.lang.invoke")

  StringConcatException is thrown by [`StringConcatFactory`](StringConcatFactory.md "class in java.lang.invoke") when linkage
  invariants are violated.

  [StringConcatFactory](StringConcatFactory.md "class in java.lang.invoke")
  ...
  [SwitchPoint](SwitchPoint.md "class in java.lang.invoke")

  A `SwitchPoint` is an object which can publish state transitions to other threads.

  [TypeDescriptor](TypeDescriptor.md "interface in java.lang.invoke")

  An entity that has a type descriptor.

  [TypeDescriptor.OfField](TypeDescriptor.OfField.md "interface in java.lang.invoke")<F extends [TypeDescriptor.OfField](TypeDescriptor.OfField.md "interface in java.lang.invoke")<F>>

  An entity that has a field type descriptor.

  [TypeDescriptor.OfMethod](TypeDescriptor.OfMethod.md "interface in java.lang.invoke")<F extends [TypeDescriptor.OfField](TypeDescriptor.OfField.md "interface in java.lang.invoke")<F>,M extends [TypeDescriptor.OfMethod](TypeDescriptor.OfMethod.md "interface in java.lang.invoke")<F,M>>
  ...
  [VarHandle](VarHandle.md "class in java.lang.invoke")
  ...
  [VarHandle.AccessMode](VarHandle.AccessMode.md "enum class in java.lang.invoke")

  The set of access modes that specify how a variable, referenced by a
  VarHandle, is accessed.

  [VarHandle.VarHandleDesc](VarHandle.VarHandleDesc.md "class in java.lang.invoke")
