...
public non-sealed class MutableCallSite
...
The [dynamic invoker](CallSite.md#dynamicInvoker()) of a mutable call site
...
The [`syncAll`](#syncAll(java.lang.invoke.MutableCallSite%5B%5D)) operation provides a way to force threads
to accept a new target value, even if there is no other synchronization.

For target values which will be frequently updated, consider using
a [volatile call site](VolatileCallSite.md "class in java.lang.invoke") instead.
...
* ## Constructor Details
  ...
  + ### MutableCallSite
    ...
    public MutableCallSite([MethodType](MethodType.md "class in java.lang.invoke") type)

    Creates a blank call site object with the given method type.
    The initial target is set to a method handle of the given type
    which will throw an [`IllegalStateException`](../IllegalStateException.md "class in java.lang") if called.
    ...
    via a call to [`setTarget`](CallSite.md#setTarget(java.lang.invoke.MethodHandle)).
  ...
  + ### MutableCallSite
    ...
    public MutableCallSite([MethodHandle](MethodHandle.md "class in java.lang.invoke") target)
...
* ## Method Details
  ...
  + ### syncAll
    ...
    public static void syncAll([MutableCallSite](MutableCallSite.md "class in java.lang.invoke")[] sites)
    ...
    (Java supports [forward time travel](../Object.md#wait()) only.)
    ...
    - A new volatile variable `V` is created, and written by the current thread.
      As defined by the JMM, this write is a global synchronization event.- As is normal with thread-local ordering of write events,
        every action already performed by the current thread is
        taken to happen before the volatile write to `V`.
        (In some implementations, this means that the current thread
        performs a global release operation.)- Specifically, the write to the current target of `S` is
          taken to happen before the volatile write to `V`.- The volatile write to `V` is placed
            (in an implementation specific manner)
            in the global synchronization order.- Consider an arbitrary thread `T` (other than the current thread).
              ...
              (This constraint is called "synchronization-order consistency".)- The JMM specifically allows optimizing compilers to elide
                ...
                indeterminate and its read value is not used.Because of the last point, the implementation behaves as if a
