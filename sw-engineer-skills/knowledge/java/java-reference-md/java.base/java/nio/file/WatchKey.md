...
service. The key remains [`valid`](#isValid()) until:

1. It is cancelled, explicitly, by invoking its [`cancel`](#cancel())
   method, or
2. Cancelled implicitly, because the object is no longer accessible,
   or
3. By [`closing`](WatchService.md#close()) the watch service.
...
and queued so that it can be retrieved by invoking the watch service's [`poll`](WatchService.md#poll()) or [`take`](WatchService.md#take()) methods. Once
...
* ## Method Details
  ...
  + ### pollEvents
    ...
    [List](../../util/List.md "interface in java.util")<[WatchEvent](WatchEvent.md "interface in java.nio.file")<?>> pollEvents()
  ...
  + ### watchable
    ...
    [Watchable](Watchable.md "interface in java.nio.file") watchable()
