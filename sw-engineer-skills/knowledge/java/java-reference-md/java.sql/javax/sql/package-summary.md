...
1. The `DataSource` interface as an alternative to the
   ...
   connection with a data source- Connection pooling and Statement pooling- Distributed transactions- Rowsets
...
* Changes can be made to a data source's properties, which means
  ...
  something about the data source or driver changes.* Connection and Statement pooling and distributed transactions are available
...
1. Event Notification
   * `RowSetListener`  
     ...
     notification method called on it.* `RowSetEvent`  
     ...
     * `RowSetMetaData`  
       ...
       internally updated to contain information about the new columns.- The Reader/Writer Facility  
       ...
       * `RowSetInternal`  
         ...
         this information. An application does not normally invoke these methods directly.* `RowSetReader`  
           ...
           reader or directly by the method `RowSet.execute`.* `RowSetWriter`  
             A disconnected `RowSet` object that has implemented the
             ...
             + Make a connection to the data source+ Check to see whether there is a conflict, that is, whether
                 a value that has been changed in the rowset has also been changed
                 in the data source+ Write the new values to the data source if there is no conflict+ Close the connection
...
* Related Packages
  ...
  [java.sql.rowset](../../../java.sql.rowset/module-summary.md)

  [javax.sql.rowset](../../../java.sql.rowset/javax/sql/rowset/package-summary.md)
...
* All Classes and InterfacesInterfacesClasses
  ...
  [CommonDataSource](CommonDataSource.md "interface in javax.sql")

  Interface that defines the methods which are common between `DataSource`,
  `XADataSource` and `ConnectionPoolDataSource`.

  [ConnectionEvent](ConnectionEvent.md "class in javax.sql")

  An `Event` object that provides information about the
  source of a connection-related event.

  [ConnectionEventListener](ConnectionEventListener.md "interface in javax.sql")

  An object that registers to be notified of events generated by a
  `PooledConnection` object.

  [ConnectionPoolDataSource](ConnectionPoolDataSource.md "interface in javax.sql")

  A factory for `PooledConnection`
  objects.

  [DataSource](DataSource.md "interface in javax.sql")

  A factory for connections to the physical data source that this
  `DataSource` object represents.

  [PooledConnection](PooledConnection.md "interface in javax.sql")
  ...
  [XADataSource](XADataSource.md "interface in javax.sql")
